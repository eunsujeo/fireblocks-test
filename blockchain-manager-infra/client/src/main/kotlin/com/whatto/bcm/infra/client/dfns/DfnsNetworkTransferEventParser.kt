 ...
 * - 알림 ID 형식은 검사하지 않는다 — 채택 명세 판 `WebhookEvent.id`에는 형식 제약이 없다.
 */
class DfnsNetworkTransferEventParser(
    ...
    override fun parse(payload: ByteArray): NetworkTransferEvent? {
        ...
        val kind = NetworkTransferEventKind.ofVendorKind(requiredText(root, "kind")) ?: return null
        ...
        return NetworkTransferEvent(
            ...
            retryOfNotificationId = DfnsTransferRequests.optionalText(root, "retryOf", ::failure),
            observation =
    ...
    /** 명세가 1 이상 정수로 정의한 전달 시도 번호. 없으면 null이고, 있는데 형식이 다르면 사건으로 받지 않는다. */
