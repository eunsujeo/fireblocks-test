 ...
 * - 자산 키는 이동 종류(`Erc20Transfer` 등)를 자산 kind로 옮겨 등록과 **같은 규칙**으로 만든다. 모델 밖 이동 종류는 키를 만들지 않고
...
class DfnsNetworkChainEventParser(
    ...
    private fun transfer(
        ...
        // 사건과 함께 온 지갑 객체가 같은 지갑을 가리켜야 이 이동을 그 지갑의 것으로 볼 수 있다.
        ...
        val assetKind = DfnsAssetKeys.assetKindOfHistoryEvent(vendorAssetKind)
        val direction =
        ...
        return try {
            NetworkChainTransfer(
                ...
                vendorAssetId = assetKind?.let { vendorAssetId(event, vendorNetwork, it) },
                vendorAssetKind = vendorAssetKind,
                direction = direction,
                status = status,
                // 모델 밖 이동 종류는 `value`가 필수가 아니고 의미도 다르므로(NFT의 tokenId 등) 금액을 읽지 않는다.
                amountBaseUnits = assetKind?.let { requiredText(event, "value") },
