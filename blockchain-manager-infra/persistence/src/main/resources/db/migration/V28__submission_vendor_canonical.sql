...
-- Dfns의 회수는 조회가 아니라 "같은 본문 재제출"이라, 재구성한 본문이 최초 제출과 바이트까지 같아야 안전하다(계약13).
...
ALTER TABLE bcm_sbmt_l
  ...
  ADD COLUMN base_amt    VARCHAR(80)  NULL,
...
ALTER TABLE bcm_sbmt_l
  ADD CONSTRAINT ck_bcm_sbmt_base_amt CHECK (base_amt IS NULL OR base_amt ~ '^(0|[1-9][0-9]*)$');

ALTER TABLE bcm_sbmt_l
  ADD CONSTRAINT ck_bcm_sbmt_dcml CHECK (dcml_cnt IS NULL OR dcml_cnt BETWEEN 0 AND 255);
