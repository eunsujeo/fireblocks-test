...
- **Dfns 경로의 확정(FINALIZED)은 벤더 `Confirmed`를 근거로 한다** (2026-09-21 사용자 확정 — 2026-09-16의 "블록 깊이로 직접 계산한다"를 **대체**) — 그 결정의 전제였던 "벤더가 어떤 기준으로 `Confirmed`라 하는지 모른다"가 깨졌다. `Confirmed`는 **네트워크별 고정 confirmation delay 경과**다(Ethereum 12 · Base 50 · Solana 8). 위탁 RPC·블록 깊이 계산(`ChainHeadPort`·`BlockDepthFinality`·`bcm.evm-rpc.*`)을 **제거한다**. 확정 근거는 **`direction: Out`을 포함한 온체인 이동 사건의 `Confirmed`**이며 전송 알림(`TransferRequest.status`)만으로는 내지 않는다 — 벤더 문서가 그쪽에는 같은 delay를 말하지 않는다. **제공자 간 확정 깊이는 같지 않다** — Fireblocks는 `numOfConfirmations` ≥ `bcm.finality-confirmations.<network>`, Dfns는 벤더 delay 통과이며, 공통으로 보장하는 것은 같은 `TxStatus`·전이·이벤트 순서·멱등·복구 결과다. Fireblocks 임계를 Dfns 값에 맞추는 안은 **택하지 않는다**(신규 EVM L2의 Fireblocks 최대 DCCP 30 < Base delay 50이고, BAT 대사가 `COMPLETED`를 임계 비교 없이 확정으로 번역해 경로 간 판정이 갈린다). 관찰 컨펌 수는 **nullable로 바꾸고 확정 근거(`finalitySource`)를 함께 싣는다** — 실측치가 없는 값을 실측치처럼 내보내지 않는다. **공개 계약 변경이라 DAW-CORE 협의가 선행한다.** 적용 순서는 **이력 복구 → 확정 전환 → 기동 차단 해제**이며 복구 완료 전에는 운영을 활성화하지 않는다. 계약은 [계약13](docs/design/13-dfns-contracts.md)을 따른다.
...
- **추측 금지** — 확인 안 된 벤더 필드·동작을 코드·주석·문서에 쓰지 않는다. Fireblocks 근거는 96·97·QnA 실측/확답을 따른다. Dfns는 2026-09-14 사용자 정정에 따라 공식 버전별 OpenAPI·공식 문서를 출처/버전과 함께 계약13에 기록해 구현 근거로 사용한다. 실제 Baseline 대조·미문서화 보장·서명 원문 수용은 구분하며 모든 개발을 실측 대기로 막지 않는다.
