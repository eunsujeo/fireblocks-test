...
- **Dfns 경로의 확정(FINALIZED) 판정은 블록 깊이로 직접 계산한다** (2026-09-16 사용자 확정) — 벤더가 `Confirmed`라고 알려도 reorg로 뒤집힐 수 있으므로 그 표기를 확정 근거로 쓰지 않는다. 사건의 `blockNumber`와 위탁 RPC로 읽은 체인 head의 깊이(블록 자체가 1컨펌)를 기존 `bcm.finality-confirmations.<network>` 임계와 비교한다. head를 읽지 못하면 확정을 **보류**하고 재시도한다 — 모름을 미확정으로 바꾸지 않는다. Fireblocks 경로의 `numOfConfirmations` 비교(02 DCCP)는 그대로 둔다. 계약은 [계약13](docs/design/13-dfns-contracts.md)을 따른다.
