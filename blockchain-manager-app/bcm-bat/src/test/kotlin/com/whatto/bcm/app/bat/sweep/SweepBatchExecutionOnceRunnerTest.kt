...
class SweepBatchExecutionOnceRunnerTest {
    @Test
    fun `명시적 일회 job은 sweep 제출을 한 번 실행하고 context를 닫는다`() {
        val command = RecordingExecutionCommand()
        val context = RecordingContext()
        val runner = SweepBatchExecutionOnceRunner(command, context.proxy)
    ...
    @Test
    fun `sweep 제출이 실패하면 context를 성공 종료하지 않는다`() {
        val command = RecordingExecutionCommand(IllegalStateException("execution failed"))
        val context = RecordingContext()
        val runner = SweepBatchExecutionOnceRunner(command, context.proxy)
    ...
    private class RecordingExecutionCommand(
        private val failure: RuntimeException? = null,
    ) : SweepBatchExecutionCommand {
        var invocations = 0

        override fun execute(): SweepBatchExecutionResult {
            invocations += 1
            failure?.let { throw it }
            return SweepBatchExecutionResult.NoCandidates
        }
    }
