...
설정 이름은 `BCM_PROVIDER`(`bcm.provider`)다. 현재 Fireblocks/로컬 조건부 조립을 구현했으며 아래 Dfns 경로는 후속 구현 대상이다. 미구현 `dfns` 선택은 기동을 거절한다.
...
아래 이름은 설계 후보이며 아직 Kotlin 인터페이스를 생성한 것은 아니다. domain은 입출력·판정 계약, infra는 벤더/RPC 구현을 맡는다.
기존 `WalletVendorPort`, `VendorStatusTranslator`, `VendorNetworkFeePort`를 재사용·분리할 지점을 먼저 대조하고 중복 추상 계층을 만들지 않는다.

| 경계 후보 | 입력 → 출력 계약 | 초기 완료 조건 |
|---|---|---|
| `ChainAssetResolver` | network·환경·자산 locator → 검증한 물리 자산/정밀도·주소/계정 규칙 | 같은 symbol도 체인별 식별, EVM contract만 필수로 강제하지 않음 |
| `ChainFinalityPolicy` | 원시 관찰·유형 있는 체인 증거·정책 버전 → 업무 판정/판정 보류·근거 | 벤더 상태 번역과 구분, EVM 확인 수를 공통 필수값으로 강제하지 않음 |
| `GasSponsorshipStrategy` | 실행 의도·자산·예산 → 대납 계획/견적·지불자·실비 참조 | 원금/비용 분리, 서명/방송은 기존 실행 경계에서 한 번만 수행 |
| 실행 지원 조회 | network·asset·작업·경로·릴리스 → 구현/검증/활성 상태·거절 사유 | 조회 결과로 제출 전 차단, 집금·가속 가능 여부를 전송 지원과 별도로 판단 |
...
2026-09-14 착수 세션에서 공통 호출 시간 인터페이스와 기존 Fireblocks 연결을 구현했다. `BCM_PROVIDER`의 Fireblocks/로컬 조건부 조립과 자격·로컬 주소 검증, 기존 실행 스크립트 연결까지 구현했다. Dfns는 기동을 차단하며 어댑터·원천 DB·웹훅 계약은 후속이다. 상세는 [설계12](design/12-provider-compatibility.md)를 따른다. DDL·OpenAPI 변경·실벤더 호출은 없다.
