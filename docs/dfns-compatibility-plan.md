...
설정 이름은 `BCM_PROVIDER`(`bcm.provider`)다. 현재 Fireblocks/로컬 조건부 조립에 더해 Dfns의 계정·주소·잔액, 자산 등록 관문, 웹훅 수신 프로토콜, 웹훅 판단 워커(**입금과 전송 알림 판단 모두**)를 `bcm.provider=dfns`에서만 뜨는 조건부 빈으로 구현했다. 출금 제출 유스케이스까지 구현했다. 발신 이동 대조와 거래 조회·내부이체·Sweep·Admin 조회는 후속이다. 조립과 무관하게 `dfns` 선택은 여전히 기동을 거절한다 — 차단 해제는 Baseline 수용과 함께 사용자 결정이다.
