...
| 33 | **자산 후보 조회 fan-out과 벤더 rate limit** — Admin 검색마다 채택 네트워크별 자산을 끝까지 페이징했다. | ✅ 해결 (2026-08-24) — 채택 네트워크 자산을 일 1회/명시 one-shot으로 PostgreSQL 캐시에 원자 갱신하고, Admin은 prefix·FTS 인덱스에서 최대 50건만 찾는다. 네트워크별 `READY/STALE/NEVER_SYNCED`를 반환하며 등록 시에는 Fireblocks 주소를 다시 검증한다. |
