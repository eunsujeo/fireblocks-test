...
- Dfns 후속 매핑은 `BCM accountId → (origin, network)별 wallet → 자산 수신 주소`로 분리한다([연결 계약](13-dfns-contracts.md#계정주소-api의-후속-연결-계약)). 현재 공통 포트/회수 판정만 구현했으며 기존 vault 매핑·업무 용도·직접 집금 과제는 바꾸지 않는다.
