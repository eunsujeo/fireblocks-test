...
수신 계약은 [흐름의 감지 절](02-bcm-flow.md#감지--웹훅-수신)을 따른다: 서명 검증 → 인박스 적재 → 200 응답.
원문 바이트·해시·서명 보존은 [수신 원본 테이블](03-bcm-db.md#bcm_whk_l--수신-알림-원본)이 정본이다.
공개키 캐시 조회로 요청마다 외부 네트워크 왕복을 만들지 않는 것이 부하 설계의 전제다.
...
워커의 분류·귀속·전이·원자 기록과 relay 발행 순서는 [수신·발행 규칙](02-bcm-flow.md#감지--웹훅-수신)을 따른다.
다음은 적체와 실패 알림이 발생했을 때 추가로 확인할 내용이다.
...
**트랜잭션 이벤트는 5종**, 아래 표에서 **4종을 구독**한다. 운영 복구의 필수 이벤트도 [같은 네 종류](../runbooks/webhook-recovery.md#절차)다:
...
원자 커밋·컨슈머 dedup·발행 순서의 계약은 [수신·발행 규칙](02-bcm-flow.md#감지--웹훅-수신),
tx 잠금과 처리 상태는 [거래 운영 상태](03-bcm-db.md#bcm_tx_l--거래-운영-상태)를 따른다.
...
실제 조작 순서와 권한은 [웹훅 구독 복구 절차](../runbooks/webhook-recovery.md)를 따른다. 아래는 복구 수단의 범위와 장애 사례다.

웹훅은 유실·지연·순서 뒤바뀜이 있을 수 있다. 세 겹으로 메운다 — 앞의 둘은 벤더 기능이고, 마지막은 우리가 벤더 기록을 직접 대조하는 안전망이다.

| 복구 수단 | 무엇인가 | 언제 |
|---|---|---|
| **벤더 재시도** | 우리가 200 을 안 주면 벤더가 **자동으로 다시 보낸다** — 공식 문서는 지수 백오프 총 10회(합산 ~8시간). **실측(2026-08 [PoC](evidence/97-webhook-poc-result.md))은 분 단위 배증**: 첫 재시도 +21~60초 → 1분 → 1분 → 3분 → 6분 → …, 도착이 분 tick(:00)에 정렬. 우리가 할 일 없음 | 수신기가 잠깐 멈췄을 때 자동으로 메워짐 |
