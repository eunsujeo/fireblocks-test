...
| `stall_alrt_dttm` | **백필 표식을 넣어 `NOT NULL`로 만든다**(아래) |

##### 백필이 자동 boost를 촉발하지 않게 한다

막힘 후보 조회는 `last_pub_stcd IN ('SUBMITTED','CONFIRMED') AND last_chng_dttm <= :changedBefore AND stall_alrt_dttm IS NULL`이다
(`TxJdbcAdapter.findStallCandidates`). 백필이 만드는 행은 **`SUBMITTED` + 과거 `last_chng_dttm`**이라 조건을 그대로 만족한다.
`StallCheckJob`이 벤더 최신 관찰을 받아 `BoostEligible`로 판정하면 `BoostSubmitter.submit`이 **RBF를 실제로 제출한다** —
마이그레이션이 자금 이동을 촉발하는 것이다.

그래서 순서를 정한다.

| 순서 | 무엇 |
|---|---|
| 1 | 백필이 만드는 행은 `stall_alrt_dttm`에 **백필 표식**을 넣는다. 조회가 `IS NULL`을 요구하므로 그 자체로 제외된다. 작업을 끄는 것에 기대지 않는다 — 끄는 것을 잊거나 다른 인스턴스가 돌면 막지 못한다 |
| 2 | 백필이 끝난 뒤, 만든 행을 **벤더 단건 조회로 대사·분류**한다(종결 / 진행 중 / 벤더에 없음) |
| 3 | 분류 결과로 **자동 boost 대상이 몇 건인지 센다**. 이 수를 보고 사람이 재개를 결정한다 — 게이트다 |
| 4 | 재개를 결정한 행만 표식을 지운다. 한 번에 다 지우지 않는다 |

`bcm.stall-check.enabled=false`로 잠그는 것은 **보조 수단**이다. 표식이 1차 방어다.
...
| 백필 표식(`stall_alrt_dttm`)이 남은 건수 | 막힘 점검이 그 거래를 영영 보지 않는다 — 재개를 끝냈는지 확인한다 |
| **DAW-CORE 호환 버전 배포 완료 증거** | 아래 |
