...
- **한 제출 키에 다른 전송 ID**가 이미 붙어 있으면 원장을 건드리지 않고 충돌로 올린다. **원장에 없는 전송**은 원장·이벤트를 만들지 않는다 —
...
- **범위 밖**: 미확인·충돌의 경보 포트, 발신 온체인 이동 대조와 그에 따른 확정, RBF(`replacementId`), 이력 복구.
  `BCM_PROVIDER=dfns` 전체 기동 차단도 그대로다.
- 검증: 전체 1,330 테스트 0 실패(이번 슬라이스 13건 추가), 전체 ktlintCheck·`git diff --check` 통과. 벤더 실호출 없음.
