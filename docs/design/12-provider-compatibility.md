...
- **출금 주소는 제출 원장만으로 복원되지 않는다.** Fireblocks·로컬은 목적지를 vault ID로 넘기므로 `vndr_dst_addr`가 `NULL`이고(03 V30),
  `src_addr`는 제출 원장에 아예 없다. 근거 우선순위를 정한다.

  | 순위 | 근거 |
  |---|---|
  | 1 | `bcm_outbox_l` payload의 관찰값(`from`·`to`) — 입금과 같은 자리다 |
  | 2 | `bcm_raw_tx_l` 원문 |
  | 3 | 발신 계정의 `bcm_ntwk_wlt_m.wlt_addr`(Dfns 발신에 한해) |

  셋 다 없으면 **복원하지 않는다.** 체인 관찰 전(`SUBMITTED`) 행은 주소가 `NULL`인 것이 정상이므로 그대로 두고,
  **관찰 뒤인데도 주소가 없는 행**은 게이트가 0을 요구한다. 복원 불가능한 과거 데이터가 남으면 전환을 막고,
  예외로 열지 여부는 실제 건수를 세어 따로 정한다.
...
| `last_chng_dttm` | `rsp_dttm`과 같게 둔다 — 사실이 그렇다. 지금 시각으로 덮으면 막힘 점검이 "방금 갱신됐다"고 잘못 읽는다 |
| `stall_alrt_dttm` | `NULL` 그대로. **업무 컬럼을 표식으로 쓰지 않는다**(아래) |

##### 백필 행은 전용 hold로 주기 작업에서 떼어 둔다

백필이 만드는 행은 **`SUBMITTED` + 과거 `last_chng_dttm` + 과거 `frst_dtct_dttm`**이라, 두 주기 작업의 조건을 그대로 만족한다.
...
백필 뒤 분류가 끝나기 전에 관찰 하나만 들어와도 표식이 사라지고, `CONFIRMED`로 진전된 행은 여전히 막힘 대상이라
**사람의 재개 결정 없이 다시 자동 boost 후보가 된다.** 게이트로 세려 해도 실제 경보 시각과 구별할 수 없다.

그래서 **전용 hold 집합**을 둔다 — 일반 관찰이 절대 건드리지 않는 곳이다.
...
**수치 검사**는 모두 0이어야 하고, 그와 별개로 아래 **준비 조건**이 충족돼야 전환한다.
하나라도 남으면 목록·단건이 조용히 거래를 빠뜨리거나, 소비자가 못 받는 응답이 나간다.
...
| 합성한 행의 `frst_dtct_dttm <> bcm_sbmt_l.rsp_dttm` 건수 | 이 값이 곧 공개 `createdAt`과 목록 순서다. 규칙만 적고 검증하지 않으면 틀린 시각이 그대로 나간다 |
| 체인 관찰 뒤 공개 상태(`CONFIRMED`·`FINALIZED`)인데 `src_addr`·`dst_addr`이 `NULL`인 건수 | 주소 없는 확정 거래를 공개한다 |
| **hold에 남은 건수** | 재개를 끝냈는지 확인한다. hold에 남아 있으면 막힘 점검·대사가 그 거래를 영영 보지 않는다 |
| Q2 writer 배포 뒤 **다시 돌린** catch-up 백필의 잔여 건수 | 백필과 새 writer 사이에 들어온 거래가 남는다 |

##### 소비자가 먼저 바뀌어야 한다

게이트가 데이터 정합성만 보면 부족하다. 이번 전환은 **DAW-CORE가 먼저 대응해야 하는 공개 계약 변경**을 포함한다.

| 변경 | 소비자가 해야 할 일 |
|---|---|
| `externalTxId` 조회의 `503 SUBMIT_IN_PROGRESS` | `Retry-After` 기반 재시도 |
| 구 커서 `400` | 안정된 시작점에서 재조회, 재조회 중 `txId` 중복 제거 |
| `createdAt` 의미 | 벤더 생성 시각으로 해석하지 않기 |
| sweep·밴드S의 `404` | 없는 거래로 처리 |

순서는 **DAW-CORE 호환 버전 배포 → BCM 백필·writer 배포 → 소비자 계약 검증 → 두 제공자 동시 read 전환**이다.

**준비 조건**(수치가 아니라 참/거짓):

- DAW-CORE 호환 버전 배포 완료
- 위 네 변경에 대한 소비자 계약 검증 통과
- 백필 hold 제어 표가 비었고 전용 분류 작업이 종료
...
- **격리된 관찰은 시각도 적지 않는다** — 받아들이지 않은 관찰의 시각을 남기면 대사가 그 창으로 비교한다.
  **격리 여부와 전이 발행 여부는 다른 판정축이다.** 허용 전이 표의 `Ignore`(같은 상태 재도착·역행)는 거절이 아니라
  **발행할 상태가 없다**는 뜻이고, 그 관찰도 컨펌·hash·벤더 원어·마지막 갱신 시각을 이미 적는다 — 벤더 시각도 같이 적는다.
  적지 않으면 `SUBMITTED` 재도착만 반복되거나 이미 `FINALIZED`인 백필 행은 **허용 전이가 영영 안 와** 시각이 비고 대사에서 계속 빠진다.
  진짜 격리는 상태 머신에 **들어가기 전에** 끝난다(거래 ID 충돌·미등록 발신 → 인박스 `F`).
  **Q2 writer는 금액·주소·귀속 동일성 검사를 `TxStateMachine` 호출과 거래 행 갱신보다 먼저 한다** —
  충돌하면 그 관찰을 인박스 격리하고 거래 행·벤더 시각·outbox를 모두 보존한다(Q0의 금액·주소 충돌 fail-closed는 Q2 범위다).
