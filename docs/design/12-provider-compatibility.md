...
- **공개 read 전환 조건은 Q0이 정하지 않는다.** Q0이 확정하는 것은 "전환은 두 제공자 동시에 한다"까지다.
  데이터 잔여 조건은 [배포 전이라 대상이 없다는 결정](#q1-백필은-하지-않는다--배포-전이라-대상이-없다-2026-09-21-사용자-확정)으로 사라졌고,
  남은 조건은 **DAW-CORE 호환 버전 배포와 소비자 계약 검증**뿐이다.
...
- **code-reviewer 반영(2026-09-18)**: Q0 계약은 통과했고 **Q1 착수 조건**에서 Critical 1건·Major 3건이 나왔다.
  ① **백필이 자금 이동을 촉발할 수 있었다** — 막힘 후보 조회는 `SUBMITTED`·오래된 `last_chng_dttm`·`stall_alrt_dttm IS NULL`을 고르는데,
  백필이 만드는 행이 정확히 그 모양이다. `BoostEligible` 판정이 나면 `BoostSubmitter`가 RBF를 실제로 제출한다.
  백필 행에 **표식을 넣어 조회에서 제외**하고, 벤더 대사·분류 → 자동 boost 대상 건수 확인 → 사람 결정으로 재개하는 순서를 넣었다.
  작업을 끄는 것은 보조 수단으로만 뒀다 — 끄는 걸 잊거나 다른 인스턴스가 돌면 막지 못한다.
  ② 게이트가 **데이터 정합성만** 봤다 — DAW-CORE가 먼저 대응해야 하는 공개 계약 변경(`503`·구 커서 `400`·`createdAt` 의미·sweep `404`)의
  준비 완료를 게이트에 넣고 배포 순서를 적었다.
  ③ **출금 주소 백필 근거가 없었다** — Fireblocks는 vault ID를 넘겨 `vndr_dst_addr`가 `NULL`이고 `src_addr`는 제출 원장에 아예 없다.
  근거 우선순위(outbox → raw → 지갑 원장)를 정하고, 관찰 뒤인데 주소가 없는 행의 0건 게이트를 더했다.
  ④ `frst_dtct_dttm = rsp_dttm`을 **규칙으로만 적고 게이트가 검증하지 않았다** — 틀린 시각이 그대로 공개 `createdAt`과 목록 순서가 된다. 게이트에 넣었다.
  Minor(개수 불일치)도 고쳤다.
- **code-reviewer 2차 반영(2026-09-18)**: 내가 고른 "표식" 수단 자체가 틀렸다.
  `stall_alrt_dttm`은 **업무 컬럼**이라 상태·컨펌이 진전된 일반 관찰·`FAILED` 보류·RBF 승자 교체가 모두 `NULL`로 되돌린다.
  분류가 끝나기 전 관찰 하나만 들어와도 표식이 사라지고, `CONFIRMED`로 진전된 행은 여전히 막힘 대상이라
  **사람 결정 없이 다시 자동 boost 후보가 된다**. 게이트로 세려 해도 실제 경보 시각과 구별되지 않는다.
  일반 관찰이 건드리지 않는 **전용 hold 제어 표**로 바꾸고, 거래 행 생성과 같은 트랜잭션에 등록하도록 했다.
...
### Q1 백필은 하지 않는다 — 배포 전이라 대상이 없다 (2026-09-21 사용자 확정)

Q0은 **기존 운영 데이터가 있다는 전제**로 백필 절차·전용 hold·전환 게이트를 초안으로 적었다. 그 전제가 틀렸다.

[운영 배포 결정표](../runbooks/production-deployment-plan.md)가 **"보류 · 배포 실행 금지"**이고 DB migration 실행도 금지다.
PostgreSQL·Kafka·DNS가 모두 미정이라 **배포된 적이 없고 운영 데이터도 없다.**
막힘 점검·대사·제출 회수 배치도 전부 `enabled: false`이고 자동 boost 네트워크는 빈 목록이다.

**사용자 확정: 거래 조회 전환(Q2·Q3)을 운영 배포보다 먼저 끝낸다.** 그러면 전환 시점에 `bcm_tx_l`이 비어 있어
백필할 행도, hold할 행도, 셀 게이트도 없다. 일어날 수 없는 상황을 위해 제어 표·분류 작업·게이트 쿼리를 끌고 가지 않는다.

- **Q1은 Q1a로 끝났다** — V32(네 컬럼·`vndr_crt_dttm` nullable)·V33(목록 인덱스)과 읽기/쓰기 경로 nullable화까지다.
- **Q1b(백필·hold·게이트)는 없앤다.** 한 번 만들었다가 이 결정으로 되돌렸다 — 설계와 구현은 git 이력에 남아 있다.
- **이 결정은 순서에 달려 있다.** 운영 배포가 전환보다 먼저가 되면 다시 판단한다. 그때 필요한 것은
  ① 백필(제출 원장·outbox 근거) ② 백필 행을 주기 작업에서 떼는 hold ③ 전환 게이트이며, 근거는 이 문단의 git 이력에 있다.
- `docs/api/openapi.yaml`의 공개 계약과 02·03의 조회 계약은 **그대로**다 — 데이터 유무와 무관하다.
...
## Q2a — 관찰의 금액·주소·구분 쓰기와 동일성 격리 (2026-09-21)

[03 V32 "충돌한 관찰은 통째로 격리한다"](03-bcm-db.md#충돌한-관찰은-통째로-격리한다-fail-closed)를 네 관찰 경로에 붙였다.

### 값을 적는 곳

| 경로 | 금액 | 주소 | 구분 |
|---|---|---|---|
| Dfns 입금(온체인 이동) | 최소 단위 + 등록 정밀도로 **판단 단계에서 한 번** 환산 | 관찰의 `from`·`to` | `DEPOSIT` |
| Dfns 전송 알림 | 제출 원장(`trsf_amt`) | 목적지만 — 벤더 정본(V30). 발신 주소는 알림에 없다 | 제출 원장의 거래 구분 |
...
워커의 즉시 격리는 **판단과 같은 트랜잭션**에서 `recordFailure`를 쓴다. 그래서 검사보다 앞서 남긴 원장 갱신은
격리와 함께 커밋된다 — "아무것도 적용하지 않는다"가 깨진다. 진입점을 셋으로 나눠 이 순서를 강제한다.

| 경로 모양 | 진입점 |
|---|---|
| 한 행 | `observeConsistently` (잠금 → 검사 → 전이) |
| 한 트랜잭션에서 여러 행(발신 좌표 적용) | `observeAllConsistently` — 전부 잠가 검사한 뒤 전부 적용 |
| 판정과 전이 사이에 호출자가 같은 행을 바꾼다 | `observeConsistentlyAround(…) { 그 쓰기 }` |

**웹훅 경로에는** 검사를 건너뛰는 입구가 없다 — `TxStateService.observe`·`observeRoot`를 없앴다.
**검사 결과(토큰)도 밖으로 내보내지 않는다.** 토큰을 노출하면 호출자가 지어낼 수 있고, "이 행을 잠그고 검사했다"는
출처는 타입으로 증명되지 않는다 — 결속을 다시 확인해도 잠금 자체를 증명하지는 못한다. 그래서 순서를 호출자가
조립하지 않고 **경계가 강제한다.** **BAT 종결 관찰은 아직 우회한다**(아래 미해결).

- **여러 행을 행마다 검사·쓰기로 돌면 안 된다.** 뒤 행의 충돌로 격리할 때 앞 행의 전이가 이미 같은 트랜잭션에 실린다.
  잠금은 트랜잭션 끝까지 남으므로 전부 잠가 검사한 뒤 쓰기를 시작해도 그 사이가 벌어지지 않는다.
- **세 번째 모양은 Fireblocks vault 발신 때문에 생겼다.** `markSubmittedByObservation`은 결속만 하는 게 아니라
  root의 활성 거래를 갈아 끼우고 `tx_hash`를 비운다(`switchRoot`). 이걸 전이 뒤로 옮기면 상태 머신이
  갈아 끼우기 전 행을 보고 승자 hash를 잃는다 — 기존 boost 시험이 잡았다. 그래서 판정만 먼저 받고 전이는 다시 읽는다.
  **다시 읽으면 다시 검사한다**(아래).
...
호출자가 잊을 수 있는 자리에 두지 않는다. 비교할 행이 없는 첫 관찰에는 읽지 않는다.

**그 컬럼은 Fireblocks 행에서 비어 있었다.** V25가 Dfns seed 전용으로 뒀기 때문인데, Fireblocks는 실제로 체크섬 혼합
대소문자 주소를 주고 출금 요청의 목적지는 길이만 검증한다 — 같은 EVM 주소의 두 표기가 만나 **잘못이 없는 관찰이 격리**된다.
V32는 "EVM은 대소문자 무관"이라고 이미 정했으므로 구현이 계약을 못 지키는 상태였다.
[03 V35](03-bcm-db.md#v35-채택-네트워크는-계정자산-모델을-가진다-2026-09-22-사용자-확정)로 **채택된 네트워크는 제공자와 무관하게
모델을 갖도록** 바꿨다(채택 API 필수 입력 + DB CHECK). `chain_id`로 EVM을 추론하는 안은 권위가 둘이 되어 택하지 않았다.

EVM 주소는 **형식을 먼저 본다** — `0x`+20바이트 hex가 아니면 대소문자를 무시하지 않는다(03 V32 "형식 검증 뒤"). 모델이 없는
네트워크(미채택 후보)는 정확히 같을 때만 같다고 본다.

### 다시 읽으면 다시 검사한다

`observeConsistentlyAround`는 **첫 판정을 통과해야 호출자의 쓰기를 부른다** — 거기서 어긋나면 원장에 남는 것이 없다.
그 쓰기 뒤에는 다시 읽고 다시 검사하는데, **여기서 어긋나면 쓰기는 이미 일어난 뒤**다. 그때는 격리가 아니라
`ConflictException`으로 트랜잭션을 통째로 물린다 — 되돌리는 것은 호출자의 트랜잭션 경계다.

호출자의 쓰기는 **원장 쓰기만** 한다. 안에서 이 경계를 다시 부르면 판정과 재검사 사이에 다른 전이가 끼어들므로
재진입은 그 자리에서 막는다. 여러 행을 다루는 입구는 **같은 root를 두 번 받지 않는다** — 받으면 둘 다 같은
오래된 행으로 검사하고 두 번 전이해, 뒤 갱신이 앞 갱신을 덮거나 같은 상태가 두 번 발행된다. `SELECT ... FOR UPDATE`가 **없는 행을 잠그지 않기** 때문이다 —
판정 때 행이 없었으면 그 사이 다른 트랜잭션이 같은 root를 만들어 커밋했을 수 있고, 그대로 적용하면
금액·주소가 다른 관찰이 상태·hash·outbox를 진행시킨다. `switchRoot`가 금액·주소를 건드리지 않는다는 것만으로는 부족하다 — 문제는 **그 행이 그 사이 새로 생겼을 수 있다**는 것이다.
재검사가 어긋나면 `ConflictException`으로 **되돌린다** —
여기까지 온 호출자는 이미 결속을 썼으므로 그 자리에서 격리할 수 없다. 워커가 새 트랜잭션에서 다시 처리하면
이번에는 행이 있으므로 처음부터 충돌로 판정한다.

**채택 네트워크의 계정·자산 모델은 한 번 정하면 바뀌지 않는다**(V35) — 해제 뒤 다시 채택해도 같은 체인이고,
다른 값을 보내면 `409`다. 과거 `bcm_tx_l` 행이 그 네트워크의 현재 모델로 비교되므로, 모델이 바뀌면
같은 주소가 전과 다른 규칙으로 판정된다.
