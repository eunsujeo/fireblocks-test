...
- **활성화 게이트**: ① `bcm_tx_l` 금액 미백필 건 0 ② 두 제공자 조립이 모두 원장 조회 ③ 제출 마감 트랜잭션이 거래 행을 함께 생성
  ④ **전환 전 발급 커서 거절**(`400`)과 호출 쪽 재시작 안내가 배포에 포함
  ⑤ [03 공개 read 전환 게이트](03-bcm-db.md#공개-read-전환-게이트)의 여덟 항목이 모두 0
  ⑥ Admin 거래 조사의 `vndr_crt_dttm` null-safe 처리가 백필보다 먼저 또는 같은 배포에 포함.
  여섯이 다 서기 전에는 공개 read를 전환하지 않는다.
