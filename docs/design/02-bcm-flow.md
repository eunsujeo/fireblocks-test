...
- **tx 대사 시간축** — 실제 실행 시각에서 기본 5분의 안정화 지연을 뺀 시각을 이번 createdAt 창의 끝으로 삼는다. `bcm_job_m.last_scs_dttm`은 실행 시각이 아니라 마지막으로 끝낸 **안정화된 createdAt 창의 끝**이다. 벤더 목록과 매니저 행 모두 벤더 `createdAt`을 KST 초 단위로 저장한 `bcm_tx_l.vndr_crt_dttm`의 같은 닫힌 구간으로 비교한다. 매니저 최초 감지 시각과 벤더 생성 시각을 서로 비교하지 않는다.
