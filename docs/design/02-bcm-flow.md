...
Dfns의 논리 계정→네트워크 지갑→자산 주소는 [13의 연결 계약](13-dfns-contracts.md#계정주소-api의-후속-연결-계약)과 그 [구현](13-dfns-contracts.md#계정주소-api의-dfns-연결--구현)을 따른다.
공개 계정·주소 API의 Dfns 연결과 웹훅 판단 경로(**입금·전송 알림·발신 확정의 블록 좌표**)는 `BCM_PROVIDER=dfns`에서만 뜨는 조건부 조립으로 구현했다. 출금 제출 유스케이스도 같은 방식으로 조립했고, 발신 사건의 블록 좌표까지 이어져 출금이 확정된다. **내부이체(계정 목적지 해소·수신측 중복 입금 방지)도 구현했다.** Sweep·Admin 조회와 거래 조회는 후속이며, 조립 여부와 무관하게 `dfns` 전체 기동 차단은 유지된다. 아래 표의 공개 계약 자체는 제공자와 무관하게 같다.
...
`Confirmed`는 **네트워크별 고정 confirmation delay**(Ethereum 12 · Base 50 등) 경과를 뜻한다. 그래도 그 표기를 쓰지 않는 것은 **임계를 우리가 정할 수 없고, reorg 되돌림 알림이 없으며, 컨펌 수가 오지 않아** 지금 몇 깊이인지 알 수 없기 때문이다([계약13](13-dfns-contracts.md)). 그래서 **벤더의 확정 표기를 `FINALIZED`의 근거로 쓰지 않는다.**
...
### 신규 키 선행 검사 — 제공자 공통 (2026-09-18 사용자 확정)

**적용 순서가 먼저다.** 제출 요청이 오면 **기존 키 판정이 항상 앞선다** — 아래 [멱등 표](#멱등--같은-키를-다시-받으면)의
`req_hash` 대조와 상태별 처리를 먼저 하고, **원장에 그 키의 행이 없을 때만** 이 표의 검사를 수행한다.

순서를 뒤집으면 멱등 계약이 깨진다 — 같은 키·다른 내용은 상태와 무관하게 `409`여야 하는데 `400`으로 가려지고,
이미 `SUBMITTED`인 건은 최초 `txId`로 `202`를 받아야 하는데 거절된다. 특히 **기존 `REQUESTED`의 회수는
당시 저장한 목적지로 진행하며 현재 주소 발급 상태를 다시 검사하지 않는다** — 나갔는지 모르는 거래를 고립시키면 안 된다.

그 위에서, **새 키**에 대해 원장에 행을 만들기 전에·벤더를 부르기 전에 막는 조건이다. **제공자와 무관하게 같다** — 같은 요청이 제공자에 따라 다르게 답하면 안 된다.

| 조건 | 응답 | 근거 |
|---|---|---|
| 송신 계정과 목적지 계정이 **같다** | `400 VALIDATION_FAILED` | 같은 주소로 가는 온체인 전송이라 잔액은 그대로고 가스만 태운다. 시간이 지나도 해소되지 않는 **요청값 자체의 모순**이라 `422`가 아니다 |
| 목적지 계정이 **없다** | `404` | 기존 계약 |
| 그 `(network, symbol)`을 **지원하지 않는다** | `400` | 기존 계약 |
| 계정·자산은 유효한데 **목적지 수신 주소가 아직 없다** | `422 UNPROCESSABLE_ENTITY` | 형식이 아니라 **목적지의 준비 상태** 문제다. 주소 발급 뒤 **같은 `externalTxId`로 정상 제출**할 수 있어야 하므로 제출 원장 행도 만들지 않는다. 주소를 우리가 그 자리에서 발급해 주지 않는다 — 발급은 별도 API 계약이다 |

마지막 줄은 목적지를 **주소로만** 받는 제공자(Dfns)에서 생긴다. vault ID를 넘기는 Fireblocks 경로는 그 자리에서 주소를 고르지 않으므로 해당 없다([계약13](13-dfns-contracts.md#내부이체--확정)).

**기존 행이 있을 때의 동일 계정 요청**

| 기존 상태 | 처리 | 근거 |
|---|---|---|
| `REQUESTED` | **회수를 계속 허용**한다 | 이미 나갔는지 모르는 구간이다. 새 요청은 막되 미결 제출의 회수까지 막으면 그 거래가 고립된다 |
| `SUBMITTED` | 멱등 표 그대로 최초 `txId`로 `202` | 이미 일어난 일이다. 지금 와서 거절해도 되돌릴 수 없다 |
| `FAILED` | **재시도를 막는다**(`400`) | 회수와 달리 재시도는 **새 전송을 만드는 일**이다. 지금 금지한 요청을 다시 보낼 이유가 없다 |

즉 **회수는 열고 재시도는 막는다** — 회수는 이미 벌어진 일의 불확실성을 해소하는 것이고, 재시도는 새 자금 이동이다.
...
| `FAILED` | 같다 | 벤더가 검증으로 거절했던 건이다. 소유권을 잡고 다시 제출한다. **Dfns는 예외 — 재시도하지 않고 `422`로 거절한다**([제공자별 회수 절차](#제공자별-회수-절차-2026-09-17)) |
...
| **`409` · `422`** | 요청 자체가 거절된 것이 확실하다 — `FAILED` 로 두고 오류를 그대로 돌려준다. 행은 지우지 않는다(같은 키의 재사용을 계속 잡아야 한다). **Dfns의 회수 재제출은 예외** — 그 자리의 `409`는 진행 중 전송일 수 있어 `REQUESTED` 로 둔다([제공자별 회수 절차](#제공자별-회수-절차-2026-09-17)) |
...
### 제공자별 회수 절차 (2026-09-17)

위 "벤더에 나갔는데 우리 기록이 없을 때"의 **회수 수단은 제공자마다 다르다.** 선기록·소유권은 그대로다. **`FAILED` 판정 기준은 Dfns 회수에서 한 가지 다르다** — 회수 재제출의 `409`는 확정 거절로 읽지 않는다(아래).

- **Fireblocks** — `externalTxId` 단건 조회로 생성 여부를 확인한다(현행 그대로).
- **Dfns** — 전송 목록 조회에 `externalId` 필터가 **없다**(`limit`·`paginationToken`뿐). 대신 공식 Idempotency 계약이
  "같은 url·같은 본문(같은 `externalId`) 재제출은 처음 만들어진 엔티티를 `200`으로 돌려준다"를 보장하므로 **같은 본문 재제출이 회수 수단**이다.
  종결 상태 뒤에도 `externalId`가 영구 결속돼 새 거래를 만들지 않는다. 본문이 다르면 `409`이므로 "같은 본문"이 안전장치다.
  다만 **명확히 보장된 범위는 종결 뒤**이고 앞 제출이 **진행 중일 때의 응답은 아직 수용 항목**이라, 그 자리의 `409`를 확정 거절로 읽지 않는다(계약13).
  실패한 전송을 다시 보내려면 **새 제출 키**가 필요하다 — 같은 키 재제출은 기존 실패 엔티티를 돌려줄 뿐이다.
  그렇다고 새 키를 바로 발급하지는 않는다: 벤더 `Failed`는 시스템 실패와 **온체인 실행 실패**를 함께 뜻해 이미 체인에 나갔을 수 있고,
  그 상태에서 새 키로 보내면 같은 자금이 두 번 나간다. 그래서 위 표의 `FAILED` 재제출은 Dfns에서 **열지 않고 `422`로 거절**하며,
  새 키 발급은 **체인 종결이 확인된 경우로 제한**한다. 벤더 문의(2026-09-18)로 `Failed`가 온체인 실패와 브로드캐스트 전 실패를 **모두** 덮고
  **둘을 구분하는 필드가 없다**는 것이 확인됐으므로, 조회로 판정하지 않는다. 형태는 **취소 → 그 nonce의 결말 확정 대기 → 새 키 발급**이며
  현재 구현은 확인 수단이 없어 계속 `422`로 거절한다([계약13](13-dfns-contracts.md#failed의-뜻과-재시도-안전성--벤더-문의-결과-2026-09-18)).
- **미결 제출 점검은 제공자와 무관하게 재제출하지 않는다.** Fireblocks는 조회로 마감하지만 Dfns는 그 조회가 없으므로
  점검이 **회수하지 않고** 마지막 확인 시각·횟수만 갱신해 백오프·경보로 넘긴다. 회수는 전체 요청 문맥과 새 소유권을 가진 원 제출 경로만 한다 —
  Dfns의 재제출은 멱등이라 이중 지급은 아니지만 재제출 자체가 자금 이동 시도이므로 백그라운드에 기본으로 열지 않는다.
- Dfns의 "같은 본문"은 저장된 canonical 값에서 **결정적으로 다시 만든다**(필드 집합·순서 고정). 원문 JSON을 보관해 재사용하지 않으며,
  재구성 전에 `req_hash`로 같은 요청임을 확인한다.
  상세와 수용 항목은 [계약13](13-dfns-contracts.md#출금-제출-계약--확정).
...
## 거래 조회 — 제공자 공통 (2026-09-18 사용자 확정)

**공개 거래 조회는 BCM 원장만 읽는다.** 단건·`externalTxId`·목록 세 갈래 모두 우리가 검증하고 수용한 상태를 돌려주고,
벤더 조회는 **응답 유실 회수·웹훅 유실 대사·막힘/RBF 직전 재검증·운영 조사**에만 남는다.

### 왜 벤더 조회로는 안 되는가

제공자 하나의 사정이 아니라 **조회라는 계약 자체가 벤더 위에서 닫히지 않는다**.

| 사실 | 결과 |
|---|---|
| Dfns 전송 목록에 `externalId` 필터가 없다 | `externalTxId` 조회 경로가 아예 없다([제공자별 회수 절차](#제공자별-회수-절차-2026-09-17)) |
| Dfns 단건 조회는 `(walletId, transferId)`를 요구한다 | `txId` 하나로 부를 수 없다 |
| **Dfns 입금의 공개 `txId`는 벤더 ID가 아니라 BCM이 `txHash`+index로 만든 값**이다 | 벤더 단건 조회로 되돌릴 수 없다([계약13](13-dfns-contracts.md)) |
| 계정↔지갑 매핑이 제공자마다 다르다 | 벤더 목록을 계정별 목록으로 그대로 쓸 수 없다 |

"단건만 벤더"식 혼합도 성립하지 않는다 — 출금은 되지만 **입금이 안 된다**.

### 응답 계약

| 원장 상태 | 공개 조회 |
|---|---|
| 공개 대상(`DEPOSIT`·`WITHDRAWAL`·`INTERNAL`) 거래 행이 있다 | `200`, **BCM이 마지막으로 수용한 상태**. 벤더 최신값으로 덮지 않는다 |
| 제출이 `SUBMITTED`이고 `txId`가 확정됐다 | `200 SUBMITTED` — 제출 마감 트랜잭션이 거래 행도 함께 만들기 때문이다(아래) |
| `externalTxId`가 `REQUESTED`이고 `txId` 미확정 | **`503 SUBMIT_IN_PROGRESS`** + `Retry-After` — 제출 표와 같은 뜻이다 |
| 제출 원장이 `FAILED`이고 **수용된 거래 행이 없다** | `404` — 제출 시도는 기록됐지만 BCM이 수용한 공개 거래가 없다. 없는 `txId`를 지어내지 않는다 |
| 수용된 거래가 `FAILED`다 | `200` + `status: FAILED` — 위와 다른 사건이다. 체인에 나갔다가 실패한 거래는 공개 거래로 존재한다 |
| 거래 행이 있지만 sweep·밴드S다 | **`404`** — 단건·`externalTxId`·목록 **어디에도 없다**. 공개 조회가 아는 거래가 아니다 |
| 아무 원장도 없다 | `404` — "벤더에 없음"이 아니라 **"BCM이 수용한 거래 없음"**이다 |
| 입금 사건이 인박스 미처리(`P`)·보류 | 처리되기 전에는 **조회되지 않는다** |
| 영구 격리(`F`) | 공개 거래로 지어내지 않는다. Admin 조사·재처리 경로에서만 보인다 |

제출 원장의 `FAILED`는 "거래가 실패했다"가 아니라 **"제출 의도가 확정 거절되어 거래 엔티티가 만들어지지 않았다"**는 뜻이다.
Dfns가 같은 키 재시도를 `422`로 막는 것과 충돌하지 않는다 — `POST`의 `422`는 "이 키로 다시 자금을 옮길 수 없다"는 **명령** 계약이고,
`GET`의 `404`는 "이 키에 대응하는 수용된 공개 거래가 없다"는 **조회** 계약이다.

격리를 조회 fallback으로 메우지 않는다 — 재처리 경로는 별도 계약이고 이미 기동 차단 해제 조건이다([설계12](12-provider-compatibility.md)).

### 제출 성공이 거래 행을 만든다

지금까지 제출은 `bcm_sbmt_l`만 `SUBMITTED`로 바꾸고 **거래 행은 웹훅이 만들었다**. 원장 서빙에서는 그대로 두면
...
`createdAt`을 벤더 시각으로 두면 제공자마다 뜻이 다르다 — Fireblocks 거래 `createdAt`과 Dfns 입금 envelope date는 다른 시간축이다([03](03-bcm-db.md)).

| 값 | 뜻 |
|---|---|
| `createdAt` | **BCM이 그 거래를 처음 원장에 수용한 시각**(`frst_dtct_dttm`) |
| `lastUpdated` | BCM이 마지막 유효 관찰을 반영한 시각(`last_chng_dttm`) |
| 목록 정렬 | `(frst_dtct_dttm, vndr_tx_id)` keyset — 벤더 커서를 감싸지 않는다 |
| 계정 귀속 | 입금은 **수신 계정**, 출금·내부이체는 **송신 계정**(`bcm_tx_l.acnt_id`). 내부이체를 수신 계정 목록에 **중복 노출하지 않는다** |
| 목록 대상 | **`DEPOSIT`·`WITHDRAWAL`·`INTERNAL`만**. sweep·밴드S는 공개 조회 전체에서 제외한다 |

**출금은 고객 계정 목록에 나타나지 않는다** — 출금 풀 계정에서 나가므로 그 계정의 목록에 귀속된다.
호출 쪽이 자기 출금을 찾는 기본 경로는 `externalTxId` 조회다. `acnt_id`는 고객 소유권이 아니라 **거래 귀속·이벤트 파티션 키**라,
모든 계정 목록에서 다시 제외하면 원장에 귀속 계정을 적은 의미를 목록만 예외로 무시하게 된다.

**sweep·밴드S는 공개 조회에 없다.** 공통 `ChainEvent`를 발행하지 않는 내부 운영 계열이라, 이벤트에 없는 것이
조회에만 나타나면 DAW-CORE가 둘을 대조할 수 없다. 공개 `Transfer`의 범위는 **BCM이 수용한 `DEPOSIT`·`WITHDRAWAL`·`INTERNAL`**이다.

벤더 시각(`vndr_crt_dttm`)은 **대사 전용**으로 남는다([tx 대사 시간축](#수수료-관측--잔액--이력--대사)) — 대사는 벤더 시간축으로 비교해야 하고, 공개 조회는 우리 시간축으로 답한다.

**이것은 Fireblocks 공개 응답의 계약 변경이다.** 같은 거래의 `createdAt`이 전환 전후로 달라진다.

그래서 **전환 전에 발급한 목록 커서는 거절한다**(`400 VALIDATION_FAILED`). 커서는 벤더 `createdAt` 위치를 담고 있는데
새 정렬은 `frst_dtct_dttm` 기준이라, 옛 위치를 새 정렬에 그대로 적용하면 **누락이나 중복이 생긴다**. 정확히 옮길 대응이 없다.
커서에는 이미 판(version)이 있어 구분된다 — 호출 쪽은 커서 없이 처음부터 다시 받는다.

전환 전에 쌓인 "거래 행 없는 `SUBMITTED` 제출"은 **메울 대상이 없다** — 조회 전환을 운영 배포보다 먼저 끝내기로 했고
그 시점에 원장이 비어 있다([설계12](12-provider-compatibility.md#q1-백필은-하지-않는다--배포-전이라-대상이-없다-2026-09-21-사용자-확정)). 순서가 뒤집히면 백필이 필요해진다.

### 전환은 두 제공자를 함께 한다

설계12의 상위 계약이 "같은 API·업무·이벤트·멱등·복구"다. 한쪽만 원장 서빙으로 옮기면
**웹훅 지연 중 보이는 상태 · 격리 사건 노출 여부 · 목록 포함 범위 · `createdAt` 의미 · 벤더 장애 시 가용성 · 미매핑/RBF 제외 시점**이 제공자마다 갈린다.
구현은 단계로 나누되 **외부 공개 전환은 동시**다. 벤더 목록·단건 포트는 지우지 않는다 — 대사는 공개 조회와 별개로 도는 독립 안전장치다.

Fireblocks 쪽은 원장 조회로 가면 오히려 단순해진다 — RBF는 `bcm_tx_l`의 root 한 행과 `actv_tx_id`로 이미 접혀 있고,
자산 매핑을 통과한 시점의 `network`·`symbol`이 원장에 있으며, DB keyset cursor가 벤더 페이지 훑기와 경계 ID 집합을 대체한다.
