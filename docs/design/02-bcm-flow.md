...
`bcm_raw_tx_l` 월별 파티션은 **대상 월이 시작되기 전에 배포 역할이 선생성**한다. 애플리케이션 실행 역할에는 DDL 권한을 주지 않고 보관 배치는 이미 있는 파티션에 DML만 수행한다. 파티션이 빠졌으면 배치를 실패시켜 heartbeat 성공을 남기지 않으며, 같은 실행의 `bcm_whk_l` 정리도 하지 않는다 — 원본을 잃은 채 성공한 것처럼 보이면 안 된다.
