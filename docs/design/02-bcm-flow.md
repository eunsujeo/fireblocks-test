...
**전환 전에 거래 행이 없는 `SUBMITTED` 제출도 메워야 한다.** 지금까지 거래 행은 웹훅이 와야 생겼으므로,
이미 수용된 출금이 전환 직후 `404`가 될 수 있다. 백필이 그 행들을 만든다 — 절차는 [Q1](12-provider-compatibility.md#q1-착수-조건--초안)이 확정한다.
