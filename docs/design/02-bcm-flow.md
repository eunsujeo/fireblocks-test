...
### 제출 전 거절 — 제공자 공통 (2026-09-18 사용자 확정)

원장에 행을 만들기 전에, 벤더를 부르기 전에 막는 조건이다. **제공자와 무관하게 같다** — 같은 요청이 제공자에 따라 다르게 답하면 안 된다.
...
**전환 시 유의**: 동일 계정 거절을 켤 때 이미 `REQUESTED`로 남은 동일 계정 행이 있으면 **그 건의 회수는 계속 허용**한다 — 새 요청은 막되
미결 제출의 회수까지 막으면 "나갔는지 모르는 거래"가 고립된다.
