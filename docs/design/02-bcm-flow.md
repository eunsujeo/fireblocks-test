...
공개 계정·주소 API의 Dfns 연결과 웹훅 **입금** 판단 경로는 `BCM_PROVIDER=dfns`에서만 뜨는 조건부 조립으로 구현했다. 거래·출금 제출·Sweep·Admin 조회와 발신 이동 대조는 후속이며, 조립 여부와 무관하게 `dfns` 전체 기동 차단은 유지된다. 아래 표의 공개 계약 자체는 제공자와 무관하게 같다.
