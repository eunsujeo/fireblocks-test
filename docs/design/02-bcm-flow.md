...
공개 계정·주소 API의 Dfns 연결과 웹훅 판단 경로(**입금·전송 알림·발신 확정의 블록 좌표**)는 `BCM_PROVIDER=dfns`에서만 뜨는 조건부 조립으로 구현했다. 출금 제출 유스케이스도 같은 방식으로 조립했고, 발신 사건의 블록 좌표까지 이어져 출금이 확정된다. **내부이체(계정 목적지 해소·수신측 중복 입금 방지)도 구현했다.** Sweep·Admin 조회와 내부이체는 후속이며, 조립 여부와 무관하게 `dfns` 전체 기동 차단은 유지된다. 아래 표의 공개 계약 자체는 제공자와 무관하게 같다.
