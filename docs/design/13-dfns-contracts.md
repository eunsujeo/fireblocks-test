...
이어서 **전송 알림 판단**과 **발신 이동 대조**를 구현해 출금이 제출부터 확정까지 이어진다. 내부이체·Sweep·이력 복구·RBF 계열은 아직 구현하지 않았다.
...
| 출금의 온체인 사건 | 같은 이동이 전송 알림과 온체인 이동 사건 양쪽으로 온다 | 발신(`direction: Out`) 사건은 **새 거래를 만들지 않는다** — `(ntwk_cd, tx_hash)`로 기존 거래를 찾아 그 거래의 전이로 반영한다. hash는 유일하지 않으므로 네 조건(후보가 정확히 하나 · 제출 원장에 대응 · 관찰이 그 제출의 지갑·자산 키·최소 단위 금액·목적지와 일치 · **같은 값의 미결 제출이 없음**)이 모두 성립할 때만 붙이고, 어긋나면 중단해 운영 신호로 남긴다 — 02의 "제출 원장이 기준"을 hash 추정으로 바꾸지 않는다. 확정을 블록 깊이로 판정하므로 이 연결이 있어야 출금도 확정된다 |
| 범위 밖 | — | RBF 계열 대응(Dfns `replacementId` 계약 미정). 발신의 `tx_hash` 조회와 제출 원장 대조는 [발신 이동 대조](#발신-이동-대조--구현)로, 입금의 원장 쓰기·outbox와 판단 워커 조립은 [입금 판단](#입금-판단--구현)·[판단 워커 조립](#판단-워커-조립--구현)으로 이후 구현했다 |
...
우리가 낸 발신 거래의 `bcm_tx_l` 행을 만드는 경로다 — 이게 있어야 발신 온체인 이동 사건을 붙일 대상이 생긴다([발신 이동 대조](#발신-이동-대조--구현)의 선행 조건).
...
| **확정** | **여기서 내지 않는다.** 관찰 컨펌 수를 `0`으로 두어 번역기가 확정을 내지 않게 한다 | 전송 알림에는 `blockNumber`가 없어 블록 깊이를 계산할 수 없다(CLAUDE.md 3절). 벤더의 `Confirmed` 표기를 확정 근거로 쓰지 않는다 — 출금의 확정은 같은 거래의 온체인 이동 사건에서 난다([발신 이동 대조](#발신-이동-대조--구현)) |
...
| 범위 밖 | 미확인·충돌의 **경보 포트**, RBF(`replacementId`) 계열, 이력 복구. 발신 온체인 이동 대조와 그에 따른 확정은 [발신 이동 대조](#발신-이동-대조--구현)로 구현했다 | |
...
구현은 도메인 순수 규칙 `NetworkChainOutgoingAttachment`와 `DfnsChainEventDecision`의 발신 분기다.

**출금의 확정이 여기서 난다.** 전송 알림에는 `blockNumber`가 없어 깊이를 계산할 수 없고([전송 알림 판단](#전송-알림-판단--구현)),
블록 좌표는 온체인 이동 사건에만 있다. 그래서 이 대조가 없으면 출금은 영영 확정되지 않는다.

| 항목 | 규칙 | 근거 |
|---|---|---|
| 거래 생성 | **새 거래를 만들지 않는다.** 전송 알림이 이미 만든 거래에 붙인다 | 만들면 같은 자금이 논리 거래 둘이 된다. 발신 사건은 그 거래에 **블록 좌표를 주는 관찰**일 뿐이다 |
...
| 단일 후보 | 후보가 **여럿이면 붙이지 않고 중단**한다 | hash는 유일하지 않다(한 트랜잭션에 여러 이동). 하나를 고르는 규칙을 지어내면 **다른 거래에 남의 확정이 붙는다** |
| 관찰 대조 | 후보가 하나여도 **관찰이 그 제출의 canonical 값(03 V28: 지갑·자산 키·최소 단위 금액)과 목적지까지 일치해야** 붙인다. 목적지는 EVM 16진 주소일 때만 대소문자를 무시한다(base58은 대소문자가 값의 일부다). 저장값이 없어 증명할 수 없으면 붙이지 않는다 | 후보 수가 1인 것만으로는 이 이동이 그 제출의 것임을 증명하지 못한다 — 한 트랜잭션에 우리 이동 A·B가 있고 **A의 전송 알림만 먼저 와 있으면 B의 사건이 A에 붙는다** |
| 미결 제출 배제 | 값이 같아도 **같은 canonical의 제출이 아직 온체인 hash를 받지 못한 채 남아 있으면 붙이지 않고 보류**한다 | 벤더는 이동과 제출을 잇는 키를 주지 않는다(`WalletHistoryEvent`에 전송 요청 ID가 없고 `TransferRequest`에 이동 순번이 없다). 그쪽 전송 알림이 **hash와 함께** 오면 해소된다 — hash가 같으면 후보가 둘이 되어 중단, 다르면 이 모호함이 사라진다. 그래서 격리가 아니라 **재시도**다. 다만 해소가 보장되지는 않는다 — 전송 알림의 `txHash`는 선택이라 hash 없이 올 수 있고, 회수되지 않은 장기 `REQUESTED`도 미결로 남는다. 그때는 재시도 창을 넘겨 격리되고 운영이 본다 |
...
| 제출 원장 대응 | 후보가 하나여도 그 거래의 벤더 전송 ID가 **제출 원장에 있을 때만** 붙인다 | 02의 "제출 원장이 기준"을 hash 일치로 대체하지 않는다. 우리 지갑에서 나갔어도 우리가 낸 제출이 아닐 수 있다 |
| 후보 없음 | 거래를 만들지 않고 **재처리 가능한 상태로 남긴다**(인박스 재시도) | 전송 알림이 늦게 올 수 있다(도착 순서는 수용 항목). 여기서 만들면 거래가 둘이 되고, **처리 완료로 닫으면 뒤늦은 알림이 거래를 만들어도 이 사건을 다시 실행할 트리거가 없어 그 출금은 영영 확정되지 않는다** |
...
| 인박스 | 붙였으면 처리 완료. **후보 여럿·대응 없음·관찰 불일치는 상한을 기다리지 않고 즉시 격리**한다. **후보 없음과 미결 제출 배제(보류)는 재시도**로 남기고 상한에 닿으면 격리된다 | 앞 셋은 시간이 지나도 해소되지 않는 이상 신호다. 후보 없음은 도착 순서 때문일 수 있어 재시도가 실제로 결과를 바꾼다 — 어느 경우도 조용히 처리 완료로 닫지 않는다 |
| 범위 밖 | 후보 없음·대응 없음의 **경보 포트**, RBF(`replacementId`) 계열, `FAILED` 재시도의 새 제출 키 발급 | |

**기동 차단 해제 선행 조건(2026-09-17)**: 발신 귀속은 "한 온체인 트랜잭션 안에서 우리 canonical과 같은 이동은 우리 제출의 것"이라는 가정 위에 서 있다. 이 가정이 깨지면(벤더가 서로 다른 `TransferRequest`를 한 트랜잭션으로 합치거나, **원장 밖 요청**이 같은 트랜잭션에 섞이면) 제출 A에 다른 이동의 확정이 붙을 수 있고, 제출 원장만 보는 배제 조회로는 막지 못한다. 그래서 이 가정은 운영 수용 항목이 아니라 **`BCM_PROVIDER=dfns` 기동 차단 해제의 선행 검증 조건**이다 — 이력 조회(`GET /wallets/{walletId}/history`)로 같은 `txHash`의 이동 목록을 관찰해 확인하기 전에는 발신 확정을 운영에 열지 않는다.
원장 밖 전송 알림은 처리 완료로 소거하지 않고 **격리해 원문을 남긴다** — 소거하면 같은 트랜잭션의 이동을 나중에 판단할 근거도 사라진다.

수용 항목: 미결 제출 배제가 **정상 건을 얼마나 오래 막는지** — 전송 알림의 `txHash`가 선택이라 hash 없이 오거나 회수되지 않은 장기 `REQUESTED`가 있으면 재시도 창을 넘겨 격리된다. 실제 빈도를 보고 창·조건을 조정한다.
**Dfns가 서로 다른 `TransferRequest`를 한 온체인 트랜잭션으로 합치는 일이 실제로 있는지** — 없다면 위 "미결 제출 배제"의 보류가 불필요한 지연이므로 조건을 풀 수 있다. 이력 조회(`GET /wallets/{walletId}/history`)가 같은 `txHash`의 이동 목록을 주므로 운영 연결 후 관찰로 확인한다. 전송 알림과 온체인 이동 사건의 **도착 순서와 그 간격**. 재시도는 [03 V29](03-bcm-db.md#v29-인박스-재시도-대기--물리-저장-계약)의 backoff를 따른다 — 기본값(상한 3회·기준 30초)에서 실제 대기 창은 `30 + 60 = 약 90초`다(상한에 닿는 시도는 그 자리에서 격리되므로 마지막 대기는 쓰이지 않는다). 그 창 안에 전송 알림이 오면 붙고, 넘으면 격리돼 운영이 본다. 실제 간격이 그 창보다 길다는 것이 확인되면 `bcm.webhook-worker.retry-base-seconds`·상한을 조정하거나 재처리 경로(이력 복구)를 붙인다.
...
| 확정 | 전송 응답에는 `blockNumber`가 없다 | `numOfConfirmations` 비교 | 출금의 확정도 **온체인 이동 사건**(`direction: Out`)의 블록 깊이로 판정한다. 발신 사건은 새 거래를 만들지 않고 네 조건(후보가 정확히 하나 · 제출 원장에 대응 · 관찰이 그 제출의 지갑·자산 키·최소 단위 금액·목적지와 일치 · **같은 값의 미결 제출이 없음**)이 모두 성립할 때만 기존 거래에 붙인다(03 V26·V28) |
...
| 본문 준비 | 목적지는 **주소만** 받는다. 금액은 등록 정밀도(03 V27)로 최소 단위 정수로 환산하고, 정밀도가 없으면 거절한다 | 계정 간 내부이체·화이트리스트 지갑은 별도 계약 전이다. 단위가 뒤섞이면 조용한 금액 사고다 |
...
그래서 새 제출 키 발급은 **체인 미제출이 확인된 경우로 제한**하고, 그 확인 수단(발신 이동 대조)이 생긴 뒤에 연다.
그때까지 `FAILED` 재시도는 `UNPROCESSABLE_ENTITY`로 거절한다 — 막아 두면 운영 개입이 필요하지만, 열어 두면 같은 자금이 두 번 나갈 수 있다.
...
- **후속**: tag/memo 체인 주소 모델, 이력 복구·RBF 계열. 자산 매핑 등록·잔액은 아래 두 절로, 웹훅의 입금·전송 알림·발신 이동 대조는 위 세 절로 구현했다.
