...
| 제출 키 길이 | `externalId`는 **1~50자**다 | `ext_tx_id`는 03에서 VARCHAR(64) | 50자를 넘는 제출 키는 **자르지 않고 거절**한다. 공개 API로 들어오는 `externalTxId`가 50자를 넘으면 Dfns 경로에서 제출할 수 없다 — **DAW-CORE와 확인이 필요한 항목이다** |
...
| 멱등 | 같은 URL·본문·`externalId` 재요청은 기존 엔티티와 `200`. 같은 `externalId`로 **다른 본문/지갑**이면 `409`(`details.duplicate`). 종결(`Confirmed`/`Failed`/`Rejected`) 뒤에는 `externalId`가 그 엔티티에 영구 결속되고 재제출도 기존 엔티티를 돌려준다. 재시도는 **새 `externalId`**가 필요하다 | 제출 키(`bcm_sbmt_l.ext_tx_id`)를 `externalId`로 쓰되 50자를 넘으면 자르지 않고 거절한다(03의 키는 VARCHAR(64)). 공식 문서가 규정한 표식(`error.details.duplicate`)이 있는 `409`만 조회 없이 "같은 키 다른 내용"으로 판정해 `Conflict` 결과(수신 바이트·`details.duplicate.id` 포함)로 돌려주고 자동 재제출하지 않는다. **표식 없는 `409`는 원인을 단정하지 않고** 일반 벤더 오류로 전파한다(다른 409 원인의 존재는 미확인). 실패한 전송의 재시도를 같은 키로 만들지 않는다 |
