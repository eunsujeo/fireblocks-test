...
우리가 낸 발신 거래의 `bcm_tx_l` 행을 만드는 경로다 — 이게 있어야 발신 온체인 이동 사건을 붙일 대상이 생긴다(후속 대조 슬라이스의 선행 조건).
...
| 관찰 대조 | 후보가 하나여도 **관찰이 그 제출의 canonical 값(03 V28: 지갑·최소 단위 금액)과 목적지까지 일치해야** 붙인다. 저장값이 없어 증명할 수 없으면 붙이지 않는다 | 후보 수가 1인 것만으로는 이 이동이 그 제출의 것임을 증명하지 못한다 — 한 트랜잭션에 우리 이동 A·B가 있고 **A의 전송 알림만 먼저 와 있으면 B의 사건이 A에 붙는다** |
...
수용 항목: 전송 알림과 온체인 이동 사건의 **도착 순서**와 그 간격 — 재시도 상한 안에 전송 알림이 오지 않으면 격리되므로, 실제 간격이 상한보다 길면 상한·백오프를 조정하거나 재처리 경로(이력 복구)가 필요하다.
...
| 확정 | 전송 응답에는 `blockNumber`가 없다 | `numOfConfirmations` 비교 | 출금의 확정도 **온체인 이동 사건**(`direction: Out`)의 블록 깊이로 판정한다. 발신 사건은 새 거래를 만들지 않고 `(ntwk_cd, tx_hash)` 단일 후보 + 제출 원장 대응일 때만 기존 거래에 붙인다(03 V26) |
...
- **후속**: tag/memo 체인 주소 모델, 웹훅의 **발신 이동 대조**. 자산 매핑 등록·잔액은 아래 두 절로, 입금과 전송 알림의 웹훅 경로는 위 판단 워커·전송 알림 판단 절로 구현했다.
