...
| **자기 계정** | 송신 계정과 목적지 계정이 같으면 **`400 VALIDATION_FAILED`**로 거절한다. 벤더 호출·원장 생성 **전에** 막는다. **제공자 공통 정책**이다(2026-09-18 사용자 확정) | 같은 주소로 가는 온체인 전송이라 잔액은 그대로고 가스만 태운다. 시간이 지나도 해소되지 않는 **요청값 자체의 모순**이라 `422`가 아니라 `400`이다. 현재 Fireblocks 경로는 막지 않으므로 함께 막는다 — 같은 요청이 제공자에 따라 다르게 답하면 안 된다 |
...
- Fireblocks·로컬은 vault ID를 넘기므로 `NULL`이다. **NULL 허용 추가 전용**이라 기존 행은 그대로 둔다.
...
| 아직 없는데 **발신 주소가 우리 발급 주소**다 | **보류(재시도)** — 입금으로 확정하지 않는다 | 전송 알림이 늦으면 `tx_hash` 결속이 아직 없다. 여기서 입금을 만들면 되돌릴 수 없다 — 이벤트는 취소가 안 된다. 알림이 오면 위 행으로 해소되고, 상한까지 안 오면 격리돼 운영이 본다 |
| 발신 주소가 우리 것이 아니다 | **입금**으로 처리한다(현행 그대로) | 외부에서 온 진짜 입금이다 |

- 발신 주소 판정은 **그 `(network, symbol)`의 발급 주소 조회**를 쓴다 — 귀속이 이미 쓰는 조회다.
- 도착 순서 세 가지(`Out` 먼저 · `In` 먼저 · 전송 알림 먼저)에서 **`INTERNAL` 한 계열만** 발행되어야 한다.
