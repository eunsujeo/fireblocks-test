...
| 금액 | 등록 정밀도로 사람 단위 금액을 **판단 단계에서 한 번** 만들고(`AssetDecimals.amountOf`), 최소 단위와 그때 쓴 정밀도를 환산 근거로 함께 적는다(03 V34). 제공자와 무관하게 02 이벤트 금액의 단위는 하나다 |
| 관찰 동일성 | 거래 행에 이미 적힌 값과 어긋나면 원장·이벤트를 쓰지 않고 관찰 전체를 즉시 격리한다. **비교는 최소 단위로** 한다 |
| 발행 | 전이 표가 발행할 상태만 outbox에 적재하고(`EventType.DEPOSIT`) 알림 ID를 `traceId`로 남긴다. 발행할 상태가 없으면 이벤트도 없다. **이벤트 금액은 원장 값이다** — 매번 현재 매핑으로 다시 환산하면 정밀도가 바뀐 뒤 재처리한 같은 사건이 원장과 다른 금액을 발행한다(03 V34) |
...
| 인박스 상태 | 입금 판단 완료·미귀속(경보 후)은 처리 완료, payload 결함은 재시도/격리, **제출 원장에 없는 전송과 한 제출 키에 두 전송이 붙은 충돌은 상한을 기다리지 않고 즉시 격리**([전송 알림 판단](#전송-알림-판단--구현)의 미확인·충돌), **이미 적힌 금액·주소와 어긋나는 관찰도 즉시 격리**(03 V32 — 아래 세 판단 표의 "관찰 동일성"), 확정 임계 설정 오류는 `P`로 남겨 복구 뒤 재처리한다 — Fireblocks 경로와 같은 규율이다. **`vndr_cmpl_yn`은 남기지 않는다**(항상 `N`) — 03이 정의한 이 표식은 Fireblocks `data.status=COMPLETED`이며 원본 보관(`bcm_raw_tx_l`)의 `vndr_tx_id` 부분 index를 위한 값이다. Dfns 온체인 사건은 인박스 `vndr_tx_id`가 null이라 그 index의 대상이 아니고 Dfns 원본 보관 경로도 없다 — 의미 없는 표식을 남기지 않으며 보관 계약은 후속이다 |
...
| 관찰 동일성 | 거래 행에 이미 적힌 금액·주소와 어긋나면 **원장·이벤트·결속 무엇도 쓰지 않고 관찰 전체를 즉시 격리한다**. 검사는 벤더 전송 ID 결속(`markSubmitted`)보다 **먼저** 한다 | 03 V32 "충돌한 관찰은 통째로 격리한다". 격리는 판단과 같은 트랜잭션에서 커밋되므로 앞서 쓴 결속은 격리와 함께 남는다 |
...
| 관찰 동일성 | 후보 **전부를 먼저 잠가 검사한 뒤** 쓰기를 시작한다. 하나라도 어긋나면 아무 후보에도 적용하지 않고 즉시 격리한다 | 03 V32. 후보마다 검사·쓰기를 붙여 돌면 뒤 후보의 충돌로 격리할 때 앞 후보의 전이가 같은 트랜잭션에 실려 함께 커밋된다 |
