...
| 판단 범위 | 전송 알림(`wallet.transfer.*`)은 [전송 알림 판단](#전송-알림-판단--구현)이 맡는다(이 슬라이스 이후). 여전히 원장을 쓰지 않고 처리 완료로만 남기는 것은 **발신 이동 사건**과 미지원/미등록 자산·정밀도 없음·발신 주소 없음이다 — 발신 이동을 기존 거래에 붙이는 대조가 아직 없기 때문이며, 그래서 **`BCM_PROVIDER=dfns` 기동 차단 해제 조건에 "발신 이동 대조 구현"이 함께 걸린다.** |
| 범위 밖 | 발신 이동 사건의 기존 거래 대조, 미등록 자산·미지원 종류·미확인 전송의 경보 포트, `WebhookTransactionParser`의 Dfns 구현, 이력 복구, 기동 차단 해제. 출금 유스케이스와 전송 알림 판단은 이후 구현했다 |
