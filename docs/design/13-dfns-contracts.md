...
| 본문 준비 | 목적지는 **주소만** 받는다(계정 간 내부이체는 [내부이체](#내부이체--확정)에서 우리가 주소로 해소해 넣는다 — 계약 확정, 구현은 후속). 금액은 등록 정밀도(03 V27)로 최소 단위 정수로 환산하고, 정밀도가 없으면 거절한다 | 계정 간 내부이체·화이트리스트 지갑은 별도 계약 전이다. 단위가 뒤섞이면 조용한 금액 사고다 |
