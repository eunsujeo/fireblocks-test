...
| `Included`(블록 포함, **pre-confirmation**) · `Confirmed`(**벤더가 final로 표시**) | 깊이 ≥ 임계면 `FINALIZED`, 아니면 `CONFIRMED` | 둘 다 블록 좌표가 있는 온체인 관찰이라 **같은 깊이 판정**을 쓴다. **벤더가 `Confirmed`를 final이라고 말하지만 그 표기를 확정 근거로 쓰지 않는다**(CLAUDE.md 3절) — 어떤 깊이를 final로 보는지 문서에 없고 reorg로 뒤집힐 수 있다. 확정은 오직 블록 깊이로 내며(`bcm.finality-confirmations.<network>`와 비교) 벤더 표기는 확정의 근거도 **추가 관문도 아니다** — 관문으로 두면 `Confirmed` 알림이 늦거나 유실될 때 깊이가 차도 확정이 영영 나오지 않는다. 깊이가 임계에 못 미치면 02의 `CONFIRMED`("체인에 등장, 컨펌 누적 중 — 미확정")다 |
...
수용 항목: **`Confirmed`의 final 기준**(컨펌 수·네트워크별 차이·고객 설정 가능 여부) — 그 기준이 우리 임계와 맞거나 컨펌 수를 받을 수 있으면 위탁 RPC 의존을 없앨 수 있다([기능 문의 2번](evidence/91-dfns-feature-requests.md)),
