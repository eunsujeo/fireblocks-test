...
| **확정** | **여기서 내지 않는다.** 관찰 컨펌 수를 `0`으로 두어 번역기가 확정을 내지 않게 한다 | 전송 알림에는 `blockNumber`가 없어 블록 깊이를 계산할 수 없다(CLAUDE.md 3절). 벤더의 `Confirmed` 표기를 확정 근거로 쓰지 않는다 — 출금의 확정은 같은 거래의 온체인 이동 사건에서 난다(후속) |
...
| 범위 밖 | 미확인·충돌의 **경보 포트**, 발신 온체인 이동 대조와 그에 따른 확정, RBF(`replacementId`) 계열, 이력 복구 | |
