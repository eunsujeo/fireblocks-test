...
그래서 새 제출 키 발급은 **체인 미제출이 확인된 경우로 제한**한다. **그 확인 수단은 아직 없다** — [발신 확정의 블록 좌표](#발신-확정의-블록-좌표--구현)는 관찰된 hash가 몇 번 블록인지만 말하고, **사건이나 hash가 오지 않았다는 사실로 미제출을 증명하지는 못한다**(웹훅 유실·지연과 구분되지 않는다). 확인 수단은 따로 계약한다 — 후보는 이력 조회(`GET /wallets/{walletId}/history`)로 그 지갑의 이동을 훑거나 위탁 RPC로 제출 키의 hash를 직접 조회하는 것이며, 어느 쪽이든 **없음을 증명하는 범위**를 먼저 정해야 한다. 그 전까지는 `422`로 거절한다.
