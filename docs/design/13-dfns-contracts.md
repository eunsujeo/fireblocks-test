...
| 출금의 온체인 사건 | 같은 이동이 전송 알림과 온체인 이동 사건 양쪽으로 온다 | 발신(`direction: Out`) 사건은 **새 거래를 만들지 않는다** — `(ntwk_cd, tx_hash)`로 기존 거래를 찾아 그 거래의 전이로 반영한다. hash는 유일하지 않으므로 **후보가 정확히 하나이고, 제출 원장(`bcm_sbmt_l`)에 대응하며, 관찰이 그 제출의 지갑·자산 키·최소 단위 금액·목적지와 모두 일치할 때만** 붙이고, 여럿이거나 대응이 없거나 값이 다르면 중단해 운영 신호로 남긴다 — 02의 "제출 원장이 기준"을 hash 추정으로 바꾸지 않는다. 확정을 블록 깊이로 판정하므로 이 연결이 있어야 출금도 확정된다 |
...
| 미결 제출 배제 | 값이 같아도 **같은 canonical의 제출이 아직 온체인 hash를 받지 못한 채 남아 있으면 붙이지 않고 보류**한다 | 벤더는 이동과 제출을 잇는 키를 주지 않는다(`WalletHistoryEvent`에 전송 요청 ID가 없고 `TransferRequest`에 이동 순번이 없다). 그쪽 전송 알림이 오면 해소된다 — hash가 같으면 후보가 둘이 되어 중단, 다르면 이 모호함이 사라진다. 그래서 격리가 아니라 **재시도**다 |
| 직렬화 경계 | 후보 조회 **전에** `(network, tx_hash)` advisory lock을 잡고, 거래를 만드는 전송 알림 판단도 같은 경계를 잡는다 | `(ntwk_cd, tx_hash)`에는 유일 제약이 없어 조회와 전이 사이에 같은 hash 행이 **새로 삽입**될 수 있다 — 기존 행의 `FOR UPDATE`로는 막히지 않는다 |
...
| 확정 | 전송 응답에는 `blockNumber`가 없다 | `numOfConfirmations` 비교 | 출금의 확정도 **온체인 이동 사건**(`direction: Out`)의 블록 깊이로 판정한다. 발신 사건은 새 거래를 만들지 않고 `(ntwk_cd, tx_hash)` 단일 후보 + 제출 원장 대응 + 관찰 일치(지갑·자산 키·금액·목적지)일 때만 기존 거래에 붙인다(03 V26·V28) |
