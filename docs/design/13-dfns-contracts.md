...
**내부 대역이며 판단 워커·제출 원장·`WebhookTransactionParser`에 연결하지 않았다** — 전송 알림은 워커가 처리 완료로만 남긴다([판단 워커 조립](#판단-워커-조립--구현)).
...
위 계약을 `DfnsTransferSubmissionService`(bcm-api)로 구현했다. 02의 **선기록·소유권·벤더 호출은 트랜잭션 밖·`FAILED` 판정 기준은 그대로**이고 회수만 다르다.
...
| 본문 재구성 | 저장된 canonical 값(지갑·자산 키·목적지·최소 단위 금액·제출 키)에서 결정적으로 만든다. 원문 JSON을 보관하지 않는다 | 같은 입력이면 같은 바이트다 — "같은 본문"이 회수의 안전장치이기 때문이다 |
...
| `409`(표식 있음) | `FAILED`로 굳히고 오류를 돌려준다. 행은 지우지 않는다 | 02의 `409`·`422` 계열 — 요청 자체가 거절된 것이 확실하다. 표식 없는 `409`는 어댑터가 일반 벤더 오류로 전파해 `REQUESTED`가 유지된다 |
