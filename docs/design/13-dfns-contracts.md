...
**내부 대역이며 판단 워커·제출 원장·`WebhookTransactionParser`에 연결하지 않았다.**
...
**저장·발행·상태 번역을 하지 않는다** — 어떤 업무 대상인지만 가른다. **내부 대역이며 판단 워커에 연결하지 않았다.**
...
구현은 도메인 순수 규칙 `NetworkChainTransactionId`와 V26 index다. **내부 대역이며 원장 쓰기·판단 워커에 연결하지 않았다.**
...
| 출금의 온체인 사건 | 같은 이동이 전송 알림과 온체인 이동 사건 양쪽으로 온다 | 발신(`direction: Out`) 사건은 **새 거래를 만들지 않는다** — `tx_hash`로 기존 `xfr-…` 거래를 찾아 그 거래의 전이로 반영한다. 확정을 블록 깊이로 판정하므로 이 연결이 있어야 출금도 확정된다 |
...
수용 항목: 한 트랜잭션이 여러 이동을 담을 때 `index`가 실제로 항상 오는지(없으면 같은 hash의 이동이 하나로 합쳐진다),
