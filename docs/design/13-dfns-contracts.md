...
**실행 빈으로 등록하지 않는 내부 대역이며 인박스 워커 연결은 후속이다.**
...
| 시각 | 벤더 시각(`vndr_crt_dttm`)은 형식이 서술된 알림 `date`에서 만든다 — 사건의 `timestamp`는 형식·시간대 서술이 없다 |
...
수용 항목: 입금 사건이 `Included`→`Confirmed`로 두 번 올 때의 전이(같은 거래 ID로 합류하는지), 미등록 자산 입금의 운영 경보 수준,
