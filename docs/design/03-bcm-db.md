...
- 우리가 제출한 전송의 온체인 이동 사건은 **새 거래를 만들지 않는다** — `tx_hash`로 기존 `xfr-…` 거래를 찾아 그 거래의 전이로 반영한다(확정은 블록 깊이로 판정하므로 이 연결이 필요하다).
  hash는 유일하지 않으므로 대조는 `(ntwk_cd, tx_hash)`로 하고 **네 조건이 모두 성립할 때만** 붙인다 — 후보가 정확히 하나, 그 거래가 제출 원장(`bcm_sbmt_l.vndr_tx_id`)에 대응, 관찰이 그 제출의 지갑·자산 키·최소 단위 금액·목적지와 일치, 그리고 **같은 값의 미결 제출(hash 미기록)이 없음** — 후보 수가 1인 것만으로는 이 이동이 그 제출의 것임을 증명하지 못한다(한 트랜잭션에 우리 이동이 둘 이상일 수 있다).
