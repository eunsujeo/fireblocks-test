...
네 컬럼은 **nullable 추가 전용**이라 이 절만으로 배포할 수 있다. 그러나 **기존 행을 채우는 절차와 공개 read 전환 조건은
이 문서에서 닫지 않는다** — 실제 잔여 건수를 세고 마이그레이션을 돌려 봐야 정해지는 값들이고,
문서만으로 미리 확정하면 실행 불가능한 계약이 남는다.

Q1 슬라이스가 이 절을 채운다. 지금까지 나온 착수 조건 초안은 [설계12 Q1 착수 조건](12-provider-compatibility.md#q1-착수-조건--초안)에 있다.

**공개 read는 Q1이 끝나기 전에 전환하지 않는다.**
