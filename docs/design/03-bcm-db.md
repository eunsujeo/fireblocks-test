...
**상태: V22 생성 의도·회수·완료 연결, V23 계정 모델, 내부 생성 유스케이스, V24 응답 증적 보관, 공식 명세 기반 Dfns HTTP 어댑터, 공개 계정·주소 API 연결(논리 계정 등록·EVM 계정 모델 네트워크의 지갑 주소를 기존 `bcm_addr_m`에 저장), Dfns 데이터셋의 자산 매핑 등록·잔액 조회, V25 계정·자산 모델 컬럼과 Solana 자산 키, V26 논리 거래 식별자 계약과 온체인 hash 조회 index, V27 등록 자산 정밀도 보관, V28 제출 시점 벤더 canonical 값 보관과 Dfns 출금 제출 유스케이스, V29 인박스 재시도 대기를 구현했다. V30 제출 목적지 주소 보관과 지갑 주소 조회 index를 내부이체 구현과 함께 적용했다. 발급 시점 자산 매핑·지갑 연결 컬럼은 아래 후속 DDL 결정이다.**
...
`network`·`symbol`·상태·컨펌 수·시각)는 이미 있다. 여기에 **공개 노출 여부를 가를 거래 구분**까지 네 컬럼을 더한다.
...
`NULL → 값` 한 번만 허용하고, 값이 있으면 다시 쓰지 않는다. 그래야 늦게 온 관찰이 최초 벤더 시각을 덮지 않는다.

**행 조건이 아니라 컬럼 단위 병합이다** — `SET vndr_crt_dttm = COALESCE(vndr_crt_dttm, :vendorCreatedAt)`.
갱신문의 `WHERE`에 `vndr_crt_dttm IS NULL`을 넣으면 시각이 이미 채워진 거래는 **이후 상태·컨펌·hash·RBF 승자 갱신이 전부 0행**이 되어
충돌로 처리된다. 이 컬럼의 보존이 다른 갱신을 막으면 안 된다.
...
**온라인 생성**이다. 지금은 아직 [운영 배포 전](../runbooks/production-deployment-plan.md)이라 원장이 비어 있지만,
일반 `CREATE INDEX`는 만드는 동안 쓰기를 막아 원장이 커진 뒤에는 Fireblocks 경로까지 멈춘다 — 적용 시점이 배포 순서에 달려 있어 큰 원장을 전제로 둔다.
...
네 컬럼은 **nullable 추가 전용**이라 이 절만으로 배포할 수 있다. 채울 기존 행은 **없다** —
조회 전환을 [운영 배포](../runbooks/production-deployment-plan.md)보다 먼저 끝내기로 해서 전환 시점에 원장이 비어 있다
([설계12](12-provider-compatibility.md#q1-백필은-하지-않는다--배포-전이라-대상이-없다-2026-09-21-사용자-확정)).

값을 채우는 것은 **Q2의 공통 쓰기 경로**다 — 제출 마감이 거래 행을 만들며 적고, 웹훅 관찰이 금액·주소를 보강한다.
배포 순서가 뒤집히면 백필과 그 안전장치가 다시 필요해진다.

### V30 제출 목적지 주소 보관 — 물리 저장 계약

내부이체(`rcv_dvcd = ACCOUNT`)는 목적지가 계정이라 `rcv_vl`에 accountId가 들어간다.
그런데 Dfns의 회수는 벤더 조회가 아니라 **같은 본문 재제출**이고, 본문의 `to`는 주소다 —
지금 회수는 `rcv_vl`을 그대로 `to`로 쓰므로 내부이체를 열면 **accountId를 주소 자리에 보내게 된다**.
V28 canonical이 "제출 본문을 재구성하는 재료 한 벌"인데 `to`가 빠져 있었고, 주소 수신자만 지원하는 동안 드러나지 않았다.

| 컬럼 | 타입 | 뜻 |
|---|---|---|
| `vndr_dst_addr` | `VARCHAR(256)` NULL | 벤더 본문에 실제로 보낸 목적지 주소. 회수가 같은 본문을 다시 만들 때 쓴다 |

- `rcv_vl`은 **논리 목적지**로 그대로 둔다 — `ADDRESS`면 주소, `ACCOUNT`면 accountId. 이 값을 주소로 덮으면 "어느 계정으로 보냈나"를 잃고
  `rcv_dvcd = ACCOUNT`인데 값이 주소인 모순이 생긴다.
- 두 값이 같을 수 있다(`ADDRESS` 수신자). 그래도 따로 적는다 — 회수가 읽을 자리를 수신자 종류에 따라 바꾸지 않는다.
- **Fireblocks·로컬은 `NULL`이다** — vault ID를 넘기므로 우리가 주소를 고르지 않는다. **NULL 허용 추가 전용**이라 기존 행은 그대로 둔다.
- 폭 256은 `bcm_ntwk_wlt_m.wlt_addr`와 같다 — 같은 성질의 값이라 폭을 맞춘다.
...
재시도가 사실상 무의미했다 — Dfns 발신 이동 사건이 전송 알림보다 먼저 온 경우가 그 예다([계약13](13-dfns-contracts.md#발신-확정의-블록-좌표--구현)).
...
- 우리가 제출한 전송의 온체인 이동 사건은 **새 거래를 만들지 않는다** — `tx_hash`로 기존 `xfr-…` 거래를 찾아 그 거래의 전이로 반영한다(확정은 블록 깊이로 판정하므로 이 연결이 필요하다).
  hash는 유일하지 않으므로 조회는 `(ntwk_cd, tx_hash)`로 한다. **이 사건에서 읽는 것은 `txHash`의 블록 좌표뿐이고, 이동이 어느 제출의 것인지는 묻지 않는다**
  (2026-09-17 사용자 확정) — 벤더가 이동과 제출을 잇는 키를 주지 않아 증명할 수 없고, 값 대조로 대신하면 원장 밖 이동이 우리 제출의 확정을 만들어 낸다.
  귀속은 벤더가 전송 요청에 결속해 준 `txHash`가 이미 해결했으므로, 그 hash를 가진 **우리 발신 거래(`ext_tx_id`가 있는 행) 전부**에 같은 좌표를 적용한다 —
  여럿이어도 모두 같은 블록이라 고를 필요가 없다. 같은 hash의 입금 행은 제 사건에서 좌표를 받으므로 건드리지 않는다.
  그 hash의 발신 거래가 아직 없으면 만들지 않고 재시도로 남긴다 — 02의 "제출 원장이 기준"을 hash 추정으로 바꾸지 않는다([계약13](13-dfns-contracts.md#발신-확정의-블록-좌표--구현)).
...
- **한 번 정한 모델은 바뀌지 않는다.** 채택 UPDATE는 `COALESCE`로 기존 값을 지키고 `chain_mdl_dvcd`가 다르면 0행이 되어 `409`다.
  해제(`release`)는 `ntwk_cd`만 비우고 모델은 남기므로, **해제 뒤 다시 채택해도 같은 체인이다.**
  이게 필요한 이유는 거래 동일성 검사가 **그 네트워크의 현재 모델**을 읽기 때문이다 — 모델이 바뀌면 과거 `bcm_tx_l` 행의
  같은 주소가 전과 다른 규칙으로 비교돼, 정상 관찰을 격리하거나 다른 주소를 같다고 하게 된다.
- **멱등은 `chainModel`까지 같을 때다.** 동시에 들어온 두 채택 중 진 쪽이 남의 모델을 제 것으로 알고 `200`을 받지 않도록,
  0행 fallback도 코드와 모델을 함께 본다.
