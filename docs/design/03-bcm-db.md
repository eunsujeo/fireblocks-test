...
**상태: V22 생성 의도·회수·완료 연결, V23 계정 모델, 내부 생성 유스케이스, V24 응답 증적 보관, 공식 명세 기반 Dfns HTTP 어댑터, 공개 계정·주소 API 연결(논리 계정 등록·EVM 계정 모델 네트워크의 지갑 주소를 기존 `bcm_addr_m`에 저장), Dfns 데이터셋의 자산 매핑 등록·잔액 조회, V25 계정·자산 모델 컬럼과 Solana 자산 키, V26 논리 거래 식별자 계약과 온체인 hash 조회 index, V27 등록 자산 정밀도 보관, V28 제출 시점 벤더 canonical 값 보관과 Dfns 출금 제출 유스케이스, V29 인박스 재시도 대기를 구현했다. V30 제출 목적지 주소 보관은 내부이체 구현과 함께 적용한다. 발급 시점 자산 매핑·지갑 연결 컬럼은 아래 후속 DDL 결정이다.**
