...
**Dfns·멀티체인 전환 범위(2026-09-14):** 아래 approve+transferFrom·approve(0)·SweepLeg 계약은 현행 EVM 기준이다.
초기 EVM 구현에는 이를 보존한다. 멀티체인은 인터페이스를 먼저 마련하고 Solana 제한 권한·목적지/인자 통제·철회·원자 실행/요청별 결과·재시도 상세 구현은 후속으로 미룬다.
Solana 집금 프로그램·노드·감사는 초기 출시 조건이 아니며 해당 체인 활성화 전 완료한다.
Solana에 EVM 부분 성공이나 동일 nonce 대체를 그대로 가정하지 않는다. 정확한 집금 프로그램/권한 모델은 미확정이며,
검증 없이 지갑별 일반 전송으로 대체하거나 승인 통제를 줄이지 않는다. [전환 계획의 체인별 설계·인수 게이트](../dfns-compatibility-plan.md)를 따른다.
이 기록은 기존 컨트랙트·출시 게이트·#51 직접 집금의 미해결 계약을 자동 변경하지 않는다.
...
**고객 보내는주소라는 업무 계층을 집금과 출금의 두 물리 역할로 나눈다.** 옴니버스는 Sweep 집금·공동 재고 보관처, 복수 출금 풀은 출금 제출을 분산하는 창구다. 이는 현행 분리 설계다. 2026-09-08 사용자 후속 설명에서 설계자는 받는주소→핫 출금 풀 직접 Sweep을 의도한다고 확인했으므로, 분리 유지 확정이라는 앞선 기록은 정정한다. 목적지·등록·예약 계약의 변경은 PLAN #51에서 검토한다. 회사 주소까지 포함한 [주소와 vault 매핑](09-asset-map.md#주소와-vault-매핑)을 따른다.
...
한쪽 방향만 반영하거나 분모에만 반영한 snapshot은 유효하지 않다. 같은 조정항은 대사(외부 업무 설계 참고: `08-balance-history.md`)에도 쓴다.
