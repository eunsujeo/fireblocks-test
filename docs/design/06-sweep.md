...
**확인 필요 — 델타 미정산분을 분모에 넣나.** 온램프·오프램프는 원장을 먼저 반영하고 온체인 이동은 델타 배치로 나중에 한다. 그 사이 구간에는 **원장과 온체인이 어긋난 몫**이 생기고, 방향이 서로 반대다.
...
넣으면 원장 기준, 안 넣으면 온체인 기준이다. **한쪽만 반영하면 안 된다** — 온램프만 넣고 오프램프를 빼면 한 방향으로 치우친다. 회계·규제 판단이 필요하고, 같은 조정항이 [대사](../../블록체인매니저/설계/08-balance-history.md)에도 걸린다.
