...
| 어떤 자산을 지원하는가 | 상품 결정 |
| 소수 자릿수 | **V27부터 현재 매핑(`bcm_vndr_ast_m.dcml_cnt`)에 보관한다**(03 V27). Fireblocks 원천은 카탈로그 해소값을 저장하고 운영자 입력은 거절하며, Dfns 원천은 카탈로그가 없어 운영자가 발행사 자료와 대조해 등록한다(없으면 등록 거절). 검색용 벤더 자산 카탈로그에는 벤더 관찰값을 그대로 캐시한다 |
...
**채택할 때 계정·자산 모델을 함께 정한다**(03 V35) — `EVM`/`SOLANA`이며 제공자와 무관하다. 자산 키 생성과 거래 관찰의
주소 동일성 비교가 이 값을 쓴다. 아직 채택하지 않은 동기화 후보만 비어 있을 수 있다.
...
- **Solana 토큰은 Token Program을 운영자가 명시한다** — 같은 mint 주소로 SPL Token과 Token-2022를 구분할 수 없으므로 등록 요청 `tokenStandard`(`SPL`/`SPL_2022`)가 필수다.
  mint는 base58 32바이트 공개키 형식을 검사한다. 네이티브 SOL·EVM 자산에 표준을 붙이면 400이다. Solana 잔액은 지갑 자산 관찰의 `Spl`/`Spl2022` 항목으로 읽으며,
  그 값이 owner의 mint별 token account 합계인지는 계약13의 수용 항목이다(확정 사실 아님).
