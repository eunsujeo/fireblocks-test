...
Admin을 구현·검토하는 개발자를 위한 권한·기능·화면 계약의 정본이다.
[업무별 설계 안내](README.md) · [현재 미해결 결정](../../PLAN.md)

| 피처 | 바로가기 |
|---|---|
| 범위·인증 | [범위와 경계](#범위와-경계) · [호출 구조와 신뢰 경계](#호출-구조와-신뢰-경계) · [사용자와 권한](#사용자와-권한) · [위험 등급과 정족수](#위험-등급과-정족수) |
| 화면 공통 | [정보 구조](#정보-구조) · [디자인 토큰](#디자인-토큰) · [공통 UI·UX 규칙](#공통-uiux-규칙) |
| 설정·조사 | [네트워크·자산](#네트워크자산) · [Vault 대조](#vault-대조) · [거래 조사](#거래-조사) · [Sweep·Allowance](#sweepallowance) |
| 정책·승인 | [Sweep 컨트랙트](#sweep-컨트랙트) · [실행 정책](#실행-정책) · [밴드S](#밴드s) · [승인함](#승인함) |
| 비상·감사 | [비상 운영](#비상-운영) · [감사](#감사) |
| 로컬 개발 | [로컬 첫 실행 준비 안내](#로컬-첫-실행-준비-안내) · [로컬 Stub 시나리오 실행](#로컬-stub-시나리오-실행) · [로컬 통합 테스트 진단](#로컬-통합-테스트-진단) |
| 검증 | [API 계약 원칙](#api-계약-원칙) · [완료 기준](#완료-기준) · [미확정](#미확정) |
...
- DAW-CORE sweep 요청 ID·외부 멱등 키·network·symbol·계정 항목 수·접수/차단/처리 상태를 먼저 보여 주고,
  요청 → 요청 항목 → 실제 실행 1..N 관계를 펼쳐 볼 수 있게 한다.
- 요청 항목에는 판단 근거 `sourceEventId`와 DAW-CORE 처리 완료 여부를 표시한다. 완료되지 않은 입금 이벤트나 다른
  요청이 이미 소비한 이벤트 때문에 거절된 경우 구조화 오류 코드와 다음 조치를 보여 준다.
...
- `chainStatus`와 `itemOutcome`을 별도 열로 표시한다. 거래 FINALIZED인데 일부 항목 FAILED인 부분 성공을 하나의
  성공 배지로 뭉개지 않는다.
- sweep 결과 이벤트마다 `eventId`·broker 발행 상태·DAW-CORE 완료 확인 시각을 보여 주고, outbox `SUCCESS`를
  DAW 업무 완료로 표시하지 않는다.
...
- DAW-CORE 이벤트 완료 확인은 `eventId`를 유일한 멱등 키로 사용하고 `txId`는 조회·상관 필드로만 쓴다. Admin은
  완료 행을 생성·수정하는 수동 버튼을 제공하지 않는다.
