...
2026-09-11 커밋 전 재검증: 정적 검사와 기능 19개는 통과했다. 일괄 화면 검사 188개 중 1개는 이전 화면에서 시작한 page-43/44 이미지 요청의 `ERR_ABORTED`가 다음 화면의 오류로 집계되어 실패했다(2회 재현). 해당 원문 01·02·43·44 화면을 각각 새 페이지로 열었을 때 이미지 로딩은 모두 정상이다. 현재 검증기는 같은 브라우저 페이지를 재사용하며 요청의 시작 화면을 구분하지 않는다. 이 한계를 기록하고 테스트·오류 검사를 완화하지 않았다.
