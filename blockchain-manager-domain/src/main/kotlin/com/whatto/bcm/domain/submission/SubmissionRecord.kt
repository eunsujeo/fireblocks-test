...
data class SubmissionRecord(
    ...
    /**
     * 제출 시점에 확정한 벤더 canonical 값(03 V28·V30). 다섯은 한 벌이며 Dfns 회수가 **같은 본문**을 다시 만들 때만 쓴다 —
     * 회수 시점의 현재 자산 매핑·정밀도를 다시 읽으면 그 사이 교체된 값으로 다른 본문이 나갈 수 있다(계약13).
     * Fireblocks·로컬은 벤더 조회로 회수하므로 다섯 다 null이다.
     */
    val vendorCanonical: SubmissionVendorCanonical? = null,
)

/** 제출 본문을 결정하는 벤더 쪽 값 한 벌. 하나라도 없으면 본문을 재구성할 수 없어 통째로 다룬다(03 V28·V30의 CHECK와 같은 규약). */
data class SubmissionVendorCanonical(
    ...
    val decimals: Int,
    /**
     * 벤더 본문에 실제로 보낸 목적지 주소(03 V30). **[SubmissionRecord.recipientValue]와 다를 수 있다** —
     * 내부이체는 논리 목적지가 계정이라 `recipientValue`가 accountId다. 회수가 이 값을 쓰지 않으면 accountId를 주소 자리에 보낸다.
     */
    ...
    init {
        ...
        require(destinationAddress.isNotBlank() && destinationAddress == destinationAddress.trim()) {
            "Invalid submission destination address"
        }
    }
