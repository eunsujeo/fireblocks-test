...
/**
 * 네트워크 전송(Dfns) 제출 유스케이스의 판단 규칙(계약13 "출금 제출 계약"). 02의 선기록·소유권은 그대로 쓰고
 * **회수 수단이 다르다** — 벤더에 `externalId` 조회가 없으므로 같은 본문 재제출이 회수다.
 * `FAILED` 판정도 두 곳 다르다 — 종결 뒤 재시도를 열지 않고([decide]), 회수 재제출의 표식 있는 `409`를 확정 거절로 읽지 않는다(호출자 몫).
 *
...
object NetworkTransferSubmissionPolicy {
     ...
     * 그 상태에서 새 키로 보내면 같은 자금이 두 번 나간다. 벤더 문의(2026-09-18)로 **두 경우를 구분하는 필드가 없다**는 것이 확인됐으므로
     * 조회로는 판정할 수 없다 — 취소로 nonce의 결말을 만든 뒤 체인에서 확정을 읽는 경로가 계약된 뒤에 연다(계약13).
     */
     ...
     *
     * `externalId`는 있을 때만 본다. **제출 응답에서는 어댑터가 이미 결속을 강제**하므로 `null`이 오지 않고,
     * 이 관용은 `externalId`가 선택인 **조회 관찰**에 같은 규칙을 쓰기 위한 것이다(계약13).
     */
    fun matches(
...
enum class NetworkTransferSubmissionAction {
    ...
    /**
     * 앞선 시도의 결말을 모른다 — 같은 본문 재제출로 회수한다. 종결 뒤 재제출이 기존 엔티티를 돌려준다는 것은 공식 문서의 보장이지만,
     * **진행 중일 때의 응답은 아직 수용 항목**이라 호출자는 그 자리의 `409`를 종결로 읽지 않는다.
     */
    Recover,
