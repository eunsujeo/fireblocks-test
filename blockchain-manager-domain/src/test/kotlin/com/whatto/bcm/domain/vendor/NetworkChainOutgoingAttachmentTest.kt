...
class NetworkChainOutgoingAttachmentTest {
    ...
    fun `후보가 하나이고 제출 원장에 대응하면 그 거래에 붙인다`() {
        ...
        val result = NetworkChainOutgoingAttachment.attach(listOf(record)) { submission }
    ...
    fun `후보가 없으면 거래를 만들지 않는다`() {
        ...
        val result = NetworkChainOutgoingAttachment.attach(emptyList()) { submission() }
    ...
    fun `후보는 하나인데 제출 원장에 대응이 없으면 붙이지 않는다`() {
        ...
        val result = NetworkChainOutgoingAttachment.attach(listOf(record)) { null }
    ...
    fun `대응은 후보 거래의 벤더 전송 ID로 찾는다`() {
        ...
        NetworkChainOutgoingAttachment.attach(listOf(txRecord(vendorTxId = "xfr-9"))) {
            looked += it
            submission()
        }
