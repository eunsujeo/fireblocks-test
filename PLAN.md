...
- [ ] **다음 구현: 판단 워커 조립과 거래·Sweep·Admin** — 위 유스케이스를 인박스 워커(P/S/F·경보 포트)에 연결하고, 제출 원장(`bcm_sbmt_l`)과 출금/Sweep 유스케이스, Admin 조회의 Dfns 구현을 계약13·02·06·08에서 확정한 뒤 구현한다. 완료 뒤 `BCM_PROVIDER=dfns` 기동 차단 해제는 Baseline 수용과 함께 사용자 결정이다.
