...
- [ ] **다음 구현: 출금·발신 판단과 거래·Sweep·Admin** — 제출 원장(`bcm_sbmt_l`)의 Dfns 연결과 발신 이동 대조, 제출 원장(`bcm_sbmt_l`)과 출금/Sweep 유스케이스, Admin 조회의 Dfns 구현을 계약13·02·06·08에서 확정한 뒤 구현한다. 완료 뒤 `BCM_PROVIDER=dfns` 기동 차단 해제는 Baseline 수용과 함께 사용자 결정이다.
