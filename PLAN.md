...
- [ ] **다음 구현: Dfns 데이터셋 자산 매핑 등록과 잔액 계약** — 현행 Admin 자산 등록은 Fireblocks 카탈로그 대조에 의존하므로 Dfns 원천에서 (network, symbol)→contract 매핑을 검증·등록하는 경로와 `GET /wallets/{id}/assets` 기반 잔액 계약(VendorBalance 필드 대응)을 계약13·07·09에서 확정한 뒤 구현한다. tag/memo 체인 주소 모델과 거래·Sweep·Admin·웹훅의 Dfns 조립은 그 뒤다.
