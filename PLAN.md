...
- [ ] **다음 구현: `FAILED` 재시도와 새 제출 키** — `422`로 막아 둔 재시도를 열려면 **체인 미제출의 확인 수단부터 계약해야 한다**. 발신 사건의 블록 좌표는 관찰된 hash의 깊이만 말하고 사건이 오지 않았다는 사실로 미제출을 증명하지 못한다(웹훅 유실·지연과 구분되지 않는다) — 이력 조회나 위탁 RPC 중 무엇으로 **없음을 증명할지와 그 범위**를 먼저 정한다. 이어서 내부이체·Sweep 유스케이스와 Admin 조회의 Dfns 구현을 계약13·02·06·08에서 확정한 뒤 구현한다. 완료 뒤 `BCM_PROVIDER=dfns` 기동 차단 해제는 Baseline 수용과 함께 사용자 결정이다.
