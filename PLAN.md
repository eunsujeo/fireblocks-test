...
- [ ] **다음 구현: 생성 응답 증적의 보호 저장소와 복구 연결 검증** — 원문 접근권한·보관/조회·무결성·실패 전파 계약을 03·13에 고정하고 증적 포트의 실제 저장 어댑터를 구현한다. 내부 생성 서비스와 실제 DB를 함께 사용해 응답 유실/저장 실패·재시작 후 조회만 재개하는 경로를 검증한다. 이어 Dfns 보류/충돌 HTTP 계약을 공개 API 연결 전에 고정한다. 공식 버전별 OpenAPI를 구현 근거로 Dfns 인증·지갑 HTTP 어댑터/계약 테스트를 진행한다(계약13에 다운로드 버전·해시 기록). 실제 Baseline 대조·명세에 없는 생성 보장·서명 원문은 운영 연결 전 수용으로 구분하며, 자료 대기를 모든 개발의 선행 조건으로 두지 않는다. Dfns 기동 차단·멀티체인 실구현 후속은 유지한다.
