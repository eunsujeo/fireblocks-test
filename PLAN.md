...
코드/API 인벤토리·공통 호출 시간/웹훅/생성 재시도·Fireblocks/로컬 조립·DB 원천 대조·네트워크 지갑 원장·증적 보관 저장소와 결합 복구 검증을 구현했다. Dfns HTTP 어댑터·실제 원천 등록·실벤더 검증·운영 전환은 미완료다. Phase 15 보류는 유지한다.
...
- [ ] **다음 구현: Dfns 보류/충돌 HTTP 계약과 공식 명세 기반 인증·지갑 어댑터** — Pending/Conflict의 공개 HTTP 매핑을 OpenAPI와 수용 테스트로 고정한다. 공식 버전별 OpenAPI(계약13의 1.1018.3 해시)를 근거로 Dfns 인증(`/auth/action/init`·`/auth/action`)·지갑 생성/조회 DTO·HTTP 어댑터와 계약 테스트를 구현하고 V24 증적 저장소에 실제 응답 바이트를 연결한다. 실제 Baseline 대조·명세에 없는 생성 보장·서명 원문은 운영 연결 전 수용으로 구분하며, 자료 대기를 모든 개발의 선행 조건으로 두지 않는다. Dfns 기동 차단·멀티체인 실구현 후속은 유지한다.
