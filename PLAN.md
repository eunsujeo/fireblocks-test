...
- [ ] **T8.2 확정 원본 보관·인박스 정리** — `bcm_raw_tx_l` 파티션 생성 주체(#19)를 먼저 확정하고,
  FINALIZED 거래의 마지막 COMPLETED 웹훅 원문·수신 시 해시를 일자 파티션에 이관한 뒤 처리된 `bcm_whk_l`을 보존 기간에
  따라 정리한다. 완료: 파티션 경계·중복 실행·원문/해시 byte 동일·정리 실패 시 원본 보존 PostgreSQL 테스트 그린.
