...
- [x] **DF3.18 Dfns 상태 번역(내부 대역)** — 기존 도메인 포트 `VendorStatusTranslator`를 `DfnsStatusTranslator`로 구현했다. 전송 상태 여섯과 온체인 이동 상태 둘만 받아 `Pending`·`Executing`·`Broadcasted`→`SUBMITTED`, `Included`→`CONFIRMED`, `Failed`→`FAILED`, `Rejected`→`REJECTED`로 옮기고, `Confirmed`는 관찰의 블록 깊이가 임계 이상일 때만 `FINALIZED`다. 전송 응답에는 `blockNumber`가 없어 출금 확정도 온체인 이동 사건에서 판정한다. 대사 종결 판정은 만들지 않는다(대사 경로 부재·확정 결정 우회 방지). 목록 밖 원어는 거절한다. 빈 미등록이며 검증은 [설계12](docs/design/12-provider-compatibility.md#dfns-상태-번역-검증-2026-09-16).
