...
- [x] **DF3.18 Dfns 상태 번역(내부 대역)** — 기존 도메인 포트 `VendorStatusTranslator`를 `DfnsStatusTranslator`로 구현했다. 전송 상태 여섯과 온체인 이동 상태 둘만 받아 `Pending`·`Executing`·`Broadcasted`→`SUBMITTED`, `Failed`→`FAILED`, `Rejected`→`REJECTED`로 옮기고, 온체인 관찰 둘(`Included`·`Confirmed`)은 블록 깊이가 임계 이상일 때만 `FINALIZED`다(벤더 확인 표기를 추가 관문으로 두지 않는다). 전송 응답에는 `blockNumber`가 없어 출금 확정도 온체인 이동 사건에서 판정한다. 대사 종결 판정은 만들지 않는다(대사 경로 부재·확정 결정 우회 방지). 목록 밖 원어는 거절한다. 빈 미등록이며 검증은 [설계12](docs/design/12-provider-compatibility.md#dfns-상태-번역-검증-2026-09-16).
- [x] **DF3.19 Dfns 온체인 이동 귀속 판정(내부 대역)** — 도메인 순수 판정 `NetworkChainAttribution`과 조회 포트 `NetworkChainLedgerLookup`을 추가했다. 발신은 02대로 제출 원장 대상(`Outgoing`)으로 먼저 가르고, 미지원 이동 종류·미등록 자산·목적지 없음·미등록 주소를 예외가 아니라 결과로 가른다(Dfns 조직 지갑은 등록하지 않은 토큰도 받는다). 주소 조회는 등록 매핑의 network·symbol로 하며 매핑 network가 관찰과 어긋나면 데이터 결함으로 중단한다. 저장·발행·상태 번역은 하지 않는다. 검증은 [설계12](docs/design/12-provider-compatibility.md#dfns-온체인-이동-귀속-판정-검증-2026-09-16).
- [x] **DF3.20 Dfns 논리 거래 식별자와 온체인 hash 조회 index(V26)** — 사용자 확정(2026-09-16)에 따라 벤더 거래 ID가 없는 Dfns 입금의 논리 거래 ID를 `txHash`·`index`에서 파생한 결정적 값(`dfns-`+요약 52자)으로 만들고 `vndr_tx_id` 폭은 그대로 둔다. 출금은 벤더가 준 `xfr-…`를 쓰고, 발신 사건은 새 거래를 만들지 않고 `tx_hash`로 기존 거래에 붙인다. 파생 입력은 길이 접두로 경계를 고정하고 EVM hash만 소문자로 정규화한다. V26은 `idx_bcm_tx_hash` 추가 전용 마이그레이션이다(UNIQUE 아님). 계약은 [03 V26](docs/design/03-bcm-db.md#v26-dfns-논리-거래-식별자와-온체인-hash-조회--물리-저장-계약)·[계약13](docs/design/13-dfns-contracts.md#논리-거래-식별자--구현), 검증은 [설계12](docs/design/12-provider-compatibility.md#dfns-논리-거래-식별자와-hash-조회-index-검증-2026-09-16).
- [x] **DF3.21 등록 자산의 정밀도 보관(V27)** — 사용자 확정(2026-09-16)에 따라 이벤트 금액 환산에 쓸 정밀도를 등록 시점에 `bcm_vndr_ast_m.dcml_cnt`(NULL 허용, 0~255)에 저장한다. 벤더가 사건마다 주는 값에 의존하지 않는다 — 명세상 필수가 아니고 폐기 예정 표기가 붙었으며 벤더가 값을 바꾸면 과거·미래 금액 해석이 달라진다. 도메인 `AssetDecimals`(상한·최소 단위 표기·환산)로 등록·관찰의 규칙을 모으고, Fireblocks는 카탈로그 해소값을 저장하며 운영자 입력을 거절(`decimalsNotApplicable`), Dfns는 카탈로그가 없어 운영자 등록값을 요구한다(`decimalsRequired`). OpenAPI 0.13.0. DF3.16의 잘못된 문구(“등록 매핑에서 읽는다”)와 DF3.13이 남긴 슬라이스 컨텍스트 회귀도 함께 고쳤다. 검증은 [설계12](docs/design/12-provider-compatibility.md#등록-자산-정밀도-보관-검증-2026-09-16).
- [x] **DF3.22 Dfns 입금 판단 유스케이스(내부 대역)** — `DfnsChainEventDecision`이 관찰·귀속·번역·확정·식별자·정밀도를 하나로 이어 원장 전이와 outbox 적재를 수행한다. 입금만 원장을 쓰고 발신·미지원·미등록·미귀속·정밀도 없음은 결과로만 돌려준다. 확정은 체인 head 깊이로 내고 head 조회 실패는 확정 보류로 전파하며, 순번 없는 사건은 거래 ID를 지어내지 않는다. 제출 키·subStatus·networkStatus는 null이고 벤더 시각은 알림 `date`에서, 금액은 등록 정밀도로 만든다. 빈 미등록이며 검증은 [설계12](docs/design/12-provider-compatibility.md#dfns-입금-판단-유스케이스-검증-2026-09-16).
- [x] **DF3.23 Dfns 판단 워커 조립** — 인박스 판단을 `WebhookDecisionWork` 경계로 추상화해 제공자마다 하나만 조립한다. 기존 워커는 `fireblocks`·`local` 한정으로 좁히고 `DfnsWebhookDecisionTransaction`·`DfnsWebhookDecisionConfig`를 `dfns`에서만 만든다. `ConfiguredFinalityPolicy`는 제공자 중립 위치로 옮겨 두 제공자가 같은 임계 설정을 쓴다. 상태 번역기·체인 head는 Webhook 앱에서만 조립한다. **입금만 판단하며** 전송 알림·발신·미지원/미등록·정밀도 없음·발신 주소 없음은 처리 완료로만 남기므로 기동 차단 해제 조건에 출금·발신 판단 구현이 함께 걸린다. 검증은 [설계12](docs/design/12-provider-compatibility.md#dfns-판단-워커-조립-검증-2026-09-16).
...
- [x] **발신 확정의 블록 좌표** — 발신 온체인 사건에서 **`txHash`의 블록 좌표만** 읽어 그 hash의 우리 발신 거래 전부에 적용하고 출금의 확정을 낸다. 이동을 제출에 귀속시키지 않는다(2026-09-17 사용자 확정) — 벤더가 상관관계 키를 주지 않아 증명할 수 없고, 값 대조로 대신하면 원장 밖 이동이 우리 제출의 확정을 만든다. 귀속은 벤더가 전송 요청에 결속한 `txHash`가 해결한다. 그 hash의 발신 거래가 아직 없으면 재시도(V29 인박스 대기).
- [x] **Webhook 판단의 실제 PostgreSQL 결합 테스트** — `@DataJdbcTest` + 명시 `@Import` 슬라이스로 제출 원장 연결·`bcm_tx_l` 전이·outbox 적재·인박스 `S`의 일괄 커밋과 롤백, 중복 알림의 outbox 중복 방지, 발신 사건의 블록 좌표로 확정까지 가는 경로를 고정했다.
- [x] **내부이체 계약 확정(코드 변경 없음)** — Dfns는 본문이 `to`(주소)뿐이라 **수신 계정의 발급 주소를 우리가 해소**해 넣는다. 미발급은 `422`이고 **제출 원장 행도 만들지 않는다**(주소 발급 뒤 같은 키로 정상 제출). **자기 계정 전송 거절은 제공자 공통**으로 올렸다(2026-09-18 사용자 확정) — 지금 Fireblocks 경로가 막지 않는다. V28 canonical에 본문의 `to`가 빠져 있던 것이 내부이체에서 드러나 **V30 `vndr_dst_addr`**로 해소 주소를 따로 적고 회수와 이벤트 `to`가 그 값을 쓴다. 수신측 `In` 사건이 입금을 또 만드는 문제도 계약했다 — 같은 hash에 우리 제출 거래가 있으면 입금을 만들지 않고, 발신 주소가 우리 것인데 아직 결속이 없으면 보류한다. OpenAPI 0.14.0(`/transactions` 422·`UNPROCESSABLE_ENTITY` 일반화).
- [x] **내부이체 제출 구현** — 계정 목적지를 발급 주소로 해소해 보내고(미발급 `422`·원장 행 미생성), V30 `vndr_dst_addr`로 해소 주소를 분리해 회수와 이벤트 `to`가 그 값을 쓴다. 기존 Dfns 행은 `rcv_vl`로 백필하고 canonical 제약을 다섯 값으로 넓혔다. 기존 키 판정을 주소 해소보다 앞으로 옮겨 멱등 계약을 지키고, 자기 계정 전송 거절을 제공자 공통으로 넣었다(자기 전송일 때만 원장을 읽는다). **수신측 `In` 중복 입금 방지는 다음 슬라이스**다.
- [x] **수신측 중복 입금 방지** — 관리 계정 간 이동은 송신 `Out`·수신 `In` 양쪽으로 오므로 그대로 두면 한 번의 내부이체에 `INTERNAL`과 `DEPOSIT`이 둘 다 나간다. 같은 hash에 우리 발신 거래가 있으면 입금을 만들지 않고, 결속 전인데 발신이 **우리 네트워크 지갑**이면 보류한다(자산 발급 기록이 아니라 소유권으로 판정). 도착 순서 두 가지를 슬라이스로 고정했다.
- [ ] **다음 구현: `FAILED` 재시도와 새 제출 키** — 벤더 문의(2026-09-18)로 확인 수단의 형태가 정해졌다: `Failed`는 온체인 실패와 브로드캐스트 전 실패를 모두 덮고 **구분 필드가 없어** 조회로는 판정할 수 없다. **취소(Cancel Transfer) → 그 nonce의 결말을 블록 깊이로 확정 → 새 제출 키 발급**으로 짠다. 남은 결정은 취소의 가스 부담 주체·`replacementId` 추적, `externalId` 50자 안에서의 새 키 규칙, 벤더 키와 `ext_tx_id`의 원장 분리, 비EVM 경로(취소 대응물 없음)다. 이어서
