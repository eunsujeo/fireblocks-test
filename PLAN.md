...
- [x] **DF3.15 Dfns 웹훅 전송 사건 관찰(내부 대역)** — 공식 현재 OpenAPI 2.0.54의 `webhooks` 항목(`wallet.transfer.*` 다섯)·`WebhookEnvelopeBase`·`TransferRequest`를 근거로 도메인 포트 `NetworkTransferEventParser`(`NetworkTransferEvent`·`NetworkTransferEventKind`)와 `DfnsNetworkTransferEventParser`를 구현했다. 채택 명세 1.1018.3에는 kind별 `data` 형식이 없다는 판 차이를 계약13에 적고 실제 Baseline 본문을 수용 항목으로 남겼다. `TransferRequest` 정규화를 `DfnsTransferRequests`로 분리해 조회 어댑터와 같은 검사를 쓰며, 업무 상태는 종류가 아니라 `status`에서 읽는다. 전송이 아닌 종류는 null, 전송 종류의 형식 오류·설정 밖 네트워크는 거절한다. `DfnsWebhookProtocol`은 전송 종류에서만 `vendorTransactionId`를 채우고 어긋나도 인박스 수용을 막지 않는다. 파서 빈 미등록·판단 워커 미조립이며 검증은 [설계12](docs/design/12-provider-compatibility.md#dfns-웹훅-전송-사건-관찰-검증-2026-09-16).
- [ ] **다음 구현: 입금 감지 사건과 거래·Sweep·Admin의 Dfns 조립** — `wallet.blockchainevent.*`(`WalletHistoryEvent`) 입금 관찰, `WebhookTransactionParser`·`VendorStatusTranslator`의 Dfns 구현과 판단 워커 조립, 제출 원장(`bcm_sbmt_l`) 연결과 출금/Sweep 유스케이스, Admin 조회의 Dfns 구현을 계약13·02·06·08에서 확정한 뒤 구현한다. 완료 뒤 `BCM_PROVIDER=dfns` 기동 차단 해제는 Baseline 수용과 함께 사용자 결정이다.
