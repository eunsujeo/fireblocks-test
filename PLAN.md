...
- [x] **Webhook 판단의 실제 PostgreSQL 결합 테스트** — `@DataJdbcTest` + 명시 `@Import` 슬라이스로 제출 원장 연결·`bcm_tx_l` 전이·outbox 적재·인박스 `S`의 일괄 커밋과 롤백, 중복 알림의 outbox 중복 방지, 발신 이동이 붙어 확정까지 가는 경로를 고정했다.
