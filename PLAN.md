...
- [ ] **다음 구현: 거래·Sweep·Admin의 Dfns 조립과 웹훅 판단** — `VendorTransactionPort`(transfer 제출·조회)·Dfns `kind` 상태 번역·`WebhookTransactionParser`·Admin 조회의 Dfns 구현과 조건부 조립을 계약13·02·06·08에서 확정한 뒤 구현한다. 완료 뒤 `BCM_PROVIDER=dfns` 기동 차단 해제는 Baseline 수용과 함께 사용자 결정이다.
