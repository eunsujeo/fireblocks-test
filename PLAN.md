...
- [ ] **다음 구현: 공개 주소 API의 Dfns 연결과 조건부 조립** — 주소 발급 입구에서 논리 계정·자산 매핑 검증 뒤 네트워크 지갑 의도를 예약하고 `PROVISIONING_PENDING`/`CONFLICT` 항목을 실제로 반환한다. `bcm.dfns.*` 바인딩·서명기/클라이언트 조건부 조립·기동 검증을 추가하되 Baseline 수용 전 `BCM_PROVIDER=dfns` 기동 차단 해제는 사용자 결정으로 남긴다. 토큰 수신 주소 완료와 자산 locator 연결은 07·09와 함께 확정한다.
