...
- [ ] **다음 구현: 출금 제출 유스케이스와 발신 이동 대조** — 위 계약대로 제출 원장(`bcm_sbmt_l`) 선기록·소유권·멱등 재제출 회수를 구현하고 발신 이동 사건을 기존 거래에 붙인다. 이어서 제출 원장(`bcm_sbmt_l`)과 출금/Sweep 유스케이스, Admin 조회의 Dfns 구현을 계약13·02·06·08에서 확정한 뒤 구현한다. 완료 뒤 `BCM_PROVIDER=dfns` 기동 차단 해제는 Baseline 수용과 함께 사용자 결정이다.
